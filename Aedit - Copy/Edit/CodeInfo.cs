...
AOutput.Write(""t"" + 'c' + 1);
	...
	static CiPopupXaml _popupXaml;
	static bool _phVisible;

	internal static void ShowXamlPopup(SciCode doc, int pos16, string xaml, bool above = false) {
		_popupXaml ??= new CiPopupXaml(CiPopupXaml.UsedBy.Info, onHiddenOrDestroyed: (_,_) => _phVisible = false);
		_popupXaml.PopupWindow.Size = (500, 300); //TODO: remove and calc auto
		_popupXaml.Xaml = xaml;
		_popupXaml.Show(doc, pos16, hideIfOutside: true, above: above);
		_phVisible = true;
	}

	internal static void HideXamlPopup() {
		if (_phVisible) _popupXaml.Hide();
	}
