...
class CiXaml
{
	...
	public CiXaml() {
		_b = new StringBuilder("<Section xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation' xmlns:x='http://schemas.microsoft.com/winfx/2006/xaml'>");
	}
	...
	public void Append(string text) => _b.Append(text);
	...
	public void Separator() {
		_b.Append("<BlockUIContainer><Separator Margin='4'></Separator></BlockUIContainer>");
	}

	public void StartOverload(int index, Action<string> onClickSelect =null) {
		_b.Append("\r\n<Paragraph");
		bool selected = onClickSelect == null;
		Style(selected ? "overloadSelected" : "overload", true);
		if (!selected) { StartHyperlink(onClickSelect, index.ToStringInvariant(), false); Style("divLink", true); }
	...
	public void Bold(string text) {
		_b.Append("<Bold>").Append(text).Append("</Bold>");
	}
	...
	public void StartHyperlink(Action<string> onClick, string uri, bool gt = true) {
		//_b.Append("<Hyperlink NavigateUri=\"").Append(uri).Append('\"');
		var guid = Guid.NewGuid().ToString();
		_b.AppendFormat("<Hyperlink NavigateUri=\"{0}\" Tag=\"{1}\"", uri, guid);
		if (gt) _b.Append('>');
		_links ??= new();
		_links[guid] = onClick;
	}
	Dictionary<string, Action<string>> _links;

	public void EndHyperlink() {
		_b.Append("</Hyperlink>");
	}

	public void Hyperlink(Action<string> onClick, string uri, string text, string append = null) {
		StartHyperlink(onClick, uri);
		_b.Append(text);
		EndHyperlink();
		if (append != null) _b.Append(append);
	}
	...
	public static Section Parse(string xaml) {
		...
		foreach (var v in sec.LogicalDescendants()) {
			if (v is Hyperlink h) {
				h.RequestNavigate += _Hyperlink_RequestNavigate;
				h.Tag = _links[h.Tag as string];
			}
		...
		static void _Hyperlink_RequestNavigate(object sender, RequestNavigateEventArgs e) {
			var s = e.Uri.OriginalString;
			var onClick = (sender as Hyperlink).Tag as Action<string>;
			AOutput.Write(s, onClick);//TODO
			onClick(s);
		}
	}
	...
	public void AppendTaggedParts(IEnumerable<TaggedText> tags) {
		...
		foreach (var v in tags) {
			...
			case TextTags.LineBreak:
				//_b.Append(i == iBr + 1 ? "<div class='br2'></div>" : "<br>"); //TODO: test. Remove iBr etc if don't need.
				_b.Append("<LineBreak/>");
				iBr = i;
				continue;
			...
			case TaggedTextStyle.Code: StartSpan("code"); break;
			}
	...
	public static string FromSymbols(IEnumerable<ISymbol> symbols, int iSelect, SemanticModel model, int position, Action<string> selectOverload) {
		//APerf.First();
		var x = new CiXaml();
		x.AppendSymbols(symbols, iSelect, model, position, selectOverload);
		x.End();
		return x.ToString();
	}

	public void AppendSymbols(IEnumerable<ISymbol> symbols, int iSelect, SemanticModel model, int position, Action<string> selectOverload) {
		//APerf.First();
		ISymbol sym = null;
		int i = -1;
		foreach (var v in symbols) {
			if (++i == iSelect) sym = v;
			StartOverload(i, i == iSelect ? null : selectOverload);//TODO: test
		...
		if (sym != null) {
				...
				if ((tKind == TypeKind.Class || tKind == TypeKind.Struct || tKind == TypeKind.Interface) && sym is INamespaceOrTypeSymbol t) {
					...
					if (a != null) {
						...
						foreach (var v in a) {
							StartParagraph("div");
							Run("dot", "• ");
							if (v is IMethodSymbol m && m.MethodKind == MethodKind.StaticConstructor) Run("keyword", "static ");
			...
			void _AppendSpaceKeyword(string name) { _b.Append(' '); Run("keyword", name); }
			...
			void _AppendAttributes(ImmutableArray<AttributeData> a, string target = null, bool isParameter = false) {
				...
				foreach (var att in a) {
					...
					if (!isParameter) { _b.Append(']'); EndParagraph(); }
				}
				if (isParameter && paramAdded) { _b.Append(']'); EndParagraph(); }
			}
		}
	...
	public void AppendSymbolLinks(string helpUrl, string sourceUrl) {
		...
		if (helpUrl != null) Hyperlink(CiGoTo.OnLinkClick, helpUrl, "more info");
		if (sourceUrl != null) {
			if (helpUrl != null) _b.Append(" , ");
			Hyperlink(CiGoTo.OnLinkClick, sourceUrl, "source code");
		}
	}
	//TODO: test links above and below
	...
	public static string FromKeyword(string name) {
		...
		x.StartParagraph();
		x.Append("Links: ");
		x.Hyperlink(CiGoTo.OnLinkClick, $"{url} {name}", "more info", " , ");
		x.Hyperlink(CiGoTo.OnLinkClick, $"{url}s", "C# keywords", ".");
		x.EndParagraph();
		return x.ToString();
	}
	...
	public static FlowDocumentScrollViewer CreateControl() {
		var d = new FlowDocument {
			FontFamily = App.Wmain.FontFamily, //TODO: was Calibri
