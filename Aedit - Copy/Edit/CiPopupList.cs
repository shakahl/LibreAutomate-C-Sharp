...
class CiPopupList
{
	...
	public CiPopupXaml PopupXaml => _popupXaml;
	...
	public CiPopupList(CiCompletion compl) {
		...
		_popup = new KPopup { Size = (240, 360), Content = _panel, CloseHides = true };

		_popupXaml = new CiPopupXaml(CiPopupXaml.UsedBy.PopupList);
		_popupTimer = new ATimer(_ShowPopupXaml);
	}
	...
	void _ShowPopupXaml(ATimer t) {
		var ci = t.Tag as CiComplItem;
		var xaml = _compl.GetDescriptionXaml(ci, 0);
		if (xaml == null) return;
		_popupXaml.Xaml = xaml;
		_popupXaml.Show(Panels.Editor.ZActiveDoc, _popup.Hwnd.Rect, Dock.Right);
	}
