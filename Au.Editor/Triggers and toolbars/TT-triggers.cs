...
partial class TriggersAndToolbars {
	...
	static void _CorrectTriggerPlace() {
		...
		var m=semo.GetEnclosingSymbol<IMethodSymbol>(cd.pos16, default);
		...
		else if(md.Body.Statements.OfType<IfStatementSyntax>().FirstOrDefault() is IfStatementSyntax firstIf) pos=firstIf.GetRealFullSpan().Start;
		else pos=md.Body.CloseBraceToken.SpanStart;
		cd.sciDoc.zGoToPos(true, pos);
	}
}
