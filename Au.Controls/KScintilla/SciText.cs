...
namespace Au.Controls
{
	...
	/// A SciText object is created by KScintilla which is the C property.
	/// </summary>
	/// <remarks>
	...
	/// 
	/// Some frequently used functions are in <see cref="KScintilla"/>, not here, eg to get/set all text or to get/convert UTF-8/16 text length/position.
	/// </remarks>
	public unsafe partial class SciText
	{
		...
		/// <summary>
		/// The host KScintilla control.
		/// </summary>
		public KScintilla C => _c;
		...
		internal SciText(KScintilla sc) {
			_c = sc;
		}
		...
		/// <param name="to">If -1, uses <see cref="KScintilla.Len8"/>.</param>
		...
		public void SetText(string s, SciSetTextFlags flags = 0, bool ignoreTags = false) {
			using (new _NoUndoNotif(this, flags)) {
				if (!ignoreTags && _CanParseTags(s)) {
					ClearText();
					_c.ZTags.AddText(s, false, _c.ZInitTagsStyle == KScintilla.ZTagsStyle.AutoWithPrefix);
				} else {
					using (new _NoReadonly(this))
		...
		bool _CanParseTags(string s) {
			...
			return _c.ZInitTagsStyle switch {
				KScintilla.ZTagsStyle.AutoAlways => s.IndexOf('<') >= 0,
				KScintilla.ZTagsStyle.AutoWithPrefix => s.Starts("<>"),
				_ => false,
			};
		}
		...
		public void AppendText(string s, bool andRN, bool scroll, bool ignoreTags = false) {
			s ??= "";
			if (!ignoreTags && _CanParseTags(s)) {
				_c.ZTags.AddText(s, true, _c.ZInitTagsStyle == KScintilla.ZTagsStyle.AutoWithPrefix);
				return;
			}
		...
		/// Does not use caching like KScintilla.Text.
		/// </summary>
