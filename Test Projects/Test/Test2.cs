...
static unsafe partial class Test
{
	...
	static void TestScreenImage()
	{
		...
		////WinImage.Capture(new RECT(100, 30, 100, 100, true), s);
		////using(var b = WinImage.Capture(new RECT(100, 30, 100, 100, true))) {
		//using(var b = WinImage.Capture(w, new RECT(100, 30, 100, 100, true))) {
		...
		//using(var b = WinImage.Capture(w, r)) {
		...
		//	Print(k.Height, k.Width, k.Stride, k.PixelFormat, k.Scan0);
		...
		for(int i = 0; i < 1; i++) {
			//Perf.SpinCPU(100);
			Perf.First();
			//var u=WinImage.Find(area, bmp, colorDiff: colorDiff).OrThrow();
			var u = WinImage.Find(area, bmp, f, colorDiff: colorDiff).OrThrow();

			//var u = WinImage.Find(area, "resource:test", f, colorDiff: colorDiff).OrThrow();

			//WinImage u = null;
			//using(var b = Tests.Properties.Resources.test) {
			//	u = WinImage.Find(area, b, f, colorDiff: colorDiff).OrThrow();
			//}

			Perf.Next(); sp += Perf.Times; sp += "\r\n";
			Print(u.Rect);
			//WinImage.Test(bmp, area);
			//WinImage.Test(bmp, area, WIFlags.WindowDC);
			//Sleep(100);
		}
		Print(sp);
		...
		//	Print(b.PixelFormat);
		//}
		//using(var b = WinImage.Capture(Wnd.Find("Calculator"), new RECT(100, 30, 100, 100, true))) {
		//	Print(b.PixelFormat);
		//}
	}
	...
	static void TestScreenImage_InBitmap()
	{
		...
		w = Wnd.Find("app - Microsoft Visual Studio", "wndclass_desked_gsk");
		w.ThrowIf0();
		using(var area = WinImage.Capture(w, w.ClientRect)) {
			//ShowBitmap(area);
			var u = WinImage.Find(area, bmp, 0).OrThrow();
			Print(u.Rect);
		}
	}
	...
	static void TestScreenImage_BottomUp()
	{
		...
		//using(var b = WinImage.Capture(r)) {
		//	_PrintBitmapStride(b);
		//	//b.Save(file);
	...
	static void TestScreenImage_CaptureExample()
	{
		...
		var file = Folders.Temp + "notepad.png";
		Wnd w = Wnd.Find("* Notepad");
		using(var b = WinImage.Capture(w, w.ClientRect)) { b.Save(file); }
		Shell.Run(file);
		...
		//using(var b = WinImage.Capture(w, r, true)) { b.Save(file); }
		//Shell.Run(file);
	}
	...
	static void TestBitmapFromHbitmap()
	{
		...
		var hb = Api.LoadImage(default, file, 0, 0, 0, Api.LR_LOADFROMFILE | Api.LR_DEFAULTSIZE);
		Print(hb);
		...
		using(var b = WinImage.BitmapFromHbitmap(hb)) {
			_PrintBitmapStride(b);
		}
	...
	static void TestScreenImage2()
	{
		...
		WIFlags flags = 0;
		//flags = WIFlags.WindowDC;

		WIArea area;

		var w = Wnd.Find("Quick*");
		area = w;
		//area = new WIArea(400, 1600, 200, 200);
		//area = new WIArea(w, 1200, 0, 200, 200);
		...
		////Print(acc.Rect);
		//area = acc;
		//area = new WIArea(acc, 2, 2, 20, 16);
		//area.SetRect(2, 2, 20, 16);

		//var r = WinImage.Find(area, image, flags);
		//var r = WinImage.Wait(10, area, image, flags);
		//var r = WinImage.Wait(0.1, area, image, flags, 0, t=> { Print($"{t.ListIndex} {t.MatchIndex} {t.Rect}"); return false; });
		//Print(WinImage.WaitNot(-5, area, image, flags));
		//Print(WinImage.WaitChanged(-5, area, flags));
		//var r = WinImage.Find(area, image, flags, 0, also: t => t.MatchIndex == 1);
		//var r = WinImage.Find(area, image, flags, 0, also: t => { t.MouseClick(); 0.5.s(); return false; });
		//var r = WinImage.Find(area, image, flags|WIFlags.AllMustExist);

		//var rectList = new List<RECT>();
		////var r = WinImage.Find(area, image, flags, 0, also: t => { rectList.Add(t.Rect); return false; });
		//var r = WinImage.Find(area, image, flags|WIFlags.AllMustExist, 0, also: t => { rectList.Add(t.Rect); return true; });
		//Print(rectList);

		WinImage r = null;
		for(int i = 0; i < 1; i++) {
			r = WinImage.Find(area,
		...
		//WinImage.Find(area, "qm info.png", flags)
		//	.MouseClick();
	...
	static void TestGarbageOfWaitWindowEtc()
	{
		...
		//foreach(var v in w.AllChildren()) Print(v.ClassName, v.NameWinForms);
		//return;
		...
		for(int i = 0; i < 1; i++) {
			...
			//WinImage.Wait(-30, w, "qm info.png");
			//GC.Collect();
	...
	static void TestStringCache()
	{
		...
		//		if(d.ContainsKey(x)) Print(s, s.Length, d[x]);
		//		else d.Add(x, s);
		//	}
		//}
	...
	static void TestWndButton()
	{
		...
		Print(b);
		//b.Click();
		Print(b.GetCheckState(), b.GetCheckState(true), b.IsChecked(), b.IsChecked(true));
		//b.Check(true);
		//b.SetCheckState(1);
	...
	static void TestPrintOverloads()
	{
		var s = "stringgg";
		object o = s;
	...
	static void TestSBCache()
	{
		...
		//var d = new AuDialog("aaa", footerText: "FOOTER");
	...
	static void TestIEnumerableWnd()
	{
		...
		var af = new Acc.Finder("BUTTON", "OK");
	...
	static void TestOrThrow()
	{
		...
		//Wnd w = Wnd.Find("Quick*");
		//var u = WinImage.Find(w, Folders.ThisAppImages + "qm info.png", WIFlags.WindowDC).OrThrow();
		//var u = WinImage.Wait(-2, w, Folders.ThisAppImages + "qm info.png", WIFlags.WindowDC).OrThrow();
		//Print(u.Rect);
		//Print( WinImage.WaitNot(-2, w, Folders.ThisAppImages + "qm info.png", WIFlags.WindowDC));
		//Print( WinImage.WaitChanged(-2, w, WIFlags.WindowDC));

		//var w = Wnd.Find("QM# - Q*");
		//var w = Wnd.Find("*Internet Explorer");
		//Perf.First();
		//var a = Acc.Find(w, "BUTTON", "Home", AFFlags.Reverse);
		//Perf.NW();
		//Print(a);
	}
	...
	static void TestWndKid()
	{
		...
		//Print(Acc.Find(w, "class=button:BUTTON"));
		//Print(Acc.Find(w, "id=1103:"));
		//Print(Acc.Find(w, "class=button:CHECKBOX"));
	...
	//		//Print("window", a.Length);
	//		////for(int i = 0; i < a.Length; i++) Print(a.GetElement(i).ClassName, a.GetElement(i).Name);
	//		//for(int i = 0; i < a.Length; i++) {
	//		//	var e =a.GetElement(i);
	//		//	Print((Wnd)e.NativeWindowHandle);
	//		//	//Print(e.ClassName, e.Name);
	//		//}
	...
	static void TestWndFocus()
	{
		...
		//	Print(ok, ok?"":Native.GetErrorMessage());
		//	//Print(w.IsFocused);
		//};
		//f.ShowDialog();
	...
	static void TestIPC()
	{
		...
		//	PrintHex(m);
		//	//Print(m.ToInt64().ToString("X"), m2.ToInt64().ToString("X"));
