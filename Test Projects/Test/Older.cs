...
static partial class Test
{
	...
	static void TestAppDomainDoCallback(AppDomain d)
	{
		...
		catch(Exception e) { Print("exception executing AppDomain", e); }
		Print("DoCallBack ended");
	}

	static void TestAppDomainUnload(AppDomain d)
	{
		try { AppDomain.Unload(d); } catch(Exception e) { Print("exception unloading AppDomain", e); }
		Print("Unload ended");
	}
	...
	static void TestWildex()
	{
		...
		var x = new Wildex(p);
		Print($"i={x.IgnoreCase}, n={x.Not}, type={x.TextType}, {x.ToString()}");
		Print(x.Match(s));
#endif
	}
	...
	static void TestWndWithWildex()
	{
		...
		//Print(AuDialog.ShowTextInputEx("aa"));
	...
	{
		//if(AuDialog.Show("Text", null, TDButtons.OKCancel, DIcon.Info) != DResult.OK) return;
		Print("ok");
		...
		//switch(AuDialog.Show("Save changes?", "More info.", customButtons: "1 Save|2 Don't Save|Cancel")) {
		...
		//switch(AuDialog.Show("Save changes?",	"info", customButtons:"1 Save|2 Don't Save|Cancel")) {
		//case 1: Print("save"); break;
		//case 2: Print("don't"); break;
		//default: Print("cancel"); break;
		//}

		//AuDialog.Show("test", null, TDButtons.OKCancel | TDButtons.YesNo | TDButtons.Retry | TDButtons.Close);
		////AuDialog.Show("test", customButtons:"1 Gerai|2 Kartoti");

		//switch(AuDialog.Show("test", null, TDButtons.OKCancel)) {
		//case DResult.OK: break;
		//}

		//switch(AuDialog.Show("test", null, "1 OK|2 Cancel")) {
		//case 1: break;
		//}

		//DebugDialog("aaa");

		//switch(AuDialog.Show("Header|Comment", "1 OK|2 Cancel")) {
		//switch(AuDialog.Show("Header|Comment", "1 O|2 C|3 X")) {
		////switch(AuDialog.Show("Header|Comment", "-1 OK|-2 Cancel")) {
		...
		//if(!AuDialog.Debug("text", TDButtons.OKCancel)) return;
		//Print("OK");

		//AuDialog.ShowTextInputEx("Big text.", "Small text bbbbhhhh gggg ccc.");
		//AuDialog.ShowTextInputEx("Big text.");
		//AuDialog.ShowTextInputEx("Big text.", editType:DEdit.Multiline);
		//AuDialog.ShowTextInputEx("Big text.", "Small jjj.", editType:DEdit.Multiline);
		...
		//int i;
		//if(!AuDialog.ShowTextInput(out i, "Example.")) return;
		//Print(i);

		//var r = AuDialog.ShowTextInputEx("Header.", "Comments.", checkBox: "Check");
		//if(r.Button != DResult.OK) return;
		//Print(r.EditText, r.IsChecked);
		...
		//AuDialog.Options.DefaultTitle = "DEFAULT";
		//AuDialog.Options.RtlLayout = true;
		//AuDialog.Options.ScreenIfNoOwner = 2;
		//AuDialog.Options.TopmostIfNoOwnerWindow = true;
		//AuDialog.Options.UseAppIcon = true;
		//AuDialog.Show("text");

		//AuDialog.Show("", "TODO: start to drag panel only when mouse moved outside its caption. Move tab button without undocking.", 0, DIcon.Info, DFlags.Wider);
		//AuDialog.Show("", "WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW123456789 EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR");

		//AuDialog.ShowNoWaitEx(null, "Text.", backgroundThread:true);
		//AuDialog.ShowNoWaitEx(null, "Text.");

		//AuDialog.Show("one");
		//Print(AuDialog.Show("Text1", "Text2", TDButtons.OKCancel, DIcon.Info, DFlags.CommandLinks|DFlags.OwnerCenter, DResult.Cancel, "1 one|2 two", Wnd.FindFast("Notepad")));
		//Print(AuDialog.ShowEx("Text1", "Text2", TDButtons.OKCancel, DIcon.Info, DFlags.CommandLinks, 0,
		//	"1 one|2 two", "11 rone|12 rtwo", "check|checked", "expanded", "x|footer", "TITLE", null, 100, -10, 10));
		//AuDialog.ShowEx("", "Text <a href=\"example\">link</a>.", onLinkClick: ed => { Print(ed.LinkHref); });

		//string s; int i;

		//var d = new AuDialog("", "Aby", TDButtons.OKCancel, DIcon.Info, flags: DFlags.CommandLinks, radioButtons: "One", customButtons: "One", expandedText: "Exp");
		//d.SetExpandedText()
		//d.FlagShowProgressBar = true;
		//d.ShowDialog();

		//if(Util.LibDebug_.IsScrollLock) Print(AuDialog.ShowTextInputEx("test", "Aby", DEdit.Combo, expandedText: "one\ntwo"));
		//else Print(AuDialog.ShowTextInputEx("test", "Aby"));

		//AuDialog.Options.RtlLayout = true;
		//Print(AuDialog.ShowTextInputEx("test", "Aby"));
		//Print(AuDialog.ShowTextInputEx("test", "Aby", DEdit.Multiline, DFlags.Wider));
		//Print(AuDialog.ShowTextInputEx("test", "Aby", style:DFlags.CommandLinks, customButtons:"One\r\ntwo"));
		//Print(AuDialog.ShowTextInputEx("test", "Aby", DEdit.Combo, expandedText:"one\ntwo", radioButtons:"10One|11Two"));
		//Print(AuDialog.ShowTextInputEx("test", "Aby", DEdit.Multiline, expandedText: "one\ntwo"));
		//Print(AuDialog.ShowTextInputEx("test", "Aby", DEdit.Multiline, radioButtons:"10One|11Two"));
		//Print(AuDialog.ShowTextInputEx("test", "Aby", DEdit.Multiline, customButtons:"One\r\ntwo", style:DFlags.CommandLinks, expandedText:"one\ntwo"));
		//AuDialog.ShowTextInputEx("test", "Aby", DEdit.Combo, expandedText:"exp", footerText:"foo", onButtonClick: e =>
		// {
		//	 if(e.Button == DResult.OK) {
		...
		//		 //e.dialog.SetEditControl(DEdit.None);
		//		 //e.dialog.SetEditControl(DEdit.Text);
		//		 //e.dialog.Send.Reconstruct();
		//	 }
		// });
		//Print(s);
		//Print(AuDialog.ShowTextInputEx("test", "one\nzero\none\ntwo", DEdit.Combo, checkBox:"check", expandedText:"expanded", footerText:"footer",
		//	x:-1,y:-100, timeoutS: 15, customButtons:"1one|2two"));
		//Print(AuDialog.ShowTextInputEx("test", customButtons:"1one|2two", onButtonClick: e => { if(e.Button == 1) { e.DoNotCloseDialog = true; Print(e.EditText); e.EditText="nnnnnnnnnnnnnnn"; } }));

		//Print(AuDialog.ShowList("1one|2two|3three|Cancel", "Infooooo"));
		//Print(AuDialog.ShowListEx("1one|2two|3three|Cancel", "Infooooo", checkBox:"Check", x:-10, timeoutS:10));


		//Print(Api.GetCurrentThreadId());
		////AuDialog.ShowNoWait(e => { Print(Api.GetCurrentThreadId()); }, "Tesxt");
		...
		//f.Click += (unu, sed) =>
		//  {
		//	  //AuDialog.Show("one", ownerWindow:f);
		//	  AuDialog.ShowNoWait(e => { Print(Api.GetCurrentThreadId()); }, true, "Tesxt", ownerWindow: c);
		//	  //AuDialog.ShowNoWait(e => { Print(Api.GetCurrentThreadId()); }, false, "Tesxt", ownerWindow: f);
		//	  //AuDialog.ShowNoWait(null, true, "Tesxt", ownerWindow: f);
		//	  //AuDialog.ShowNoWait(null, true, "Tesxt");
		...
		//try {
		////Print(AuDialog.Show("Text1", "Text2", TDButtons.OKCancel, DIcon.Info, DFlags.CommandLinks));
		...
		//int i;
		//if(!AuDialog.ShowTextInput(out i, "Text1", 5, DEdit.Text)) return;
		//Print(i);

		//string s;
		//if(!AuDialog.ShowTextInput(out s, "Text2")) return;
		//Print(s, s!=null);

		//string s;
		//if(!AuDialog.ShowTextInput(out s, "Text2", editType: DEdit.Multiline)) return;
		////TODO: need resizable
		//Print(s, s != null);
	...
	static void TestTaskDialog3()
	{
		...
		//var d = new AuDialog("dddd");
		////d.SetIcon(DIcon.Info);
		...
		//AuDialog.Show("Owned", owner:Wnd.Find(null, "Notepad"));

		//var f = new Form();
		//f.Click += (unu, sed) => AuDialog.Show("Owned", owner: f);
		//f.ShowDialog();

		//Timer_.After(2000, t => MessageBox.Show("kkkk"));
		//AuDialog.Options.AutoOwnerWindow = true;
		//Timer_.After(2000, t => AuDialog.Show("kkkk"));
		//var f = new Form();
		//f.ShowDialog();

		//AuDialog d = new AuDialog("aa", "bb", "1 One|4 Four");
		...
		//Print("fin");
		//AuDialog.Show("Simple");

		//Timer_.Every(50, t => GC.Collect());

		//Print(AuDialog.Show("Simple"));
		//Print(AuDialog.Show("Simple", "", "Yes|No"));
		//Print(AuDialog.Show("Simple", "", "-1 One|-2 Two"));
		//AuDialog.Options.UseAppIcon = true;
		//Print(AuDialog.Show("Simple", "", "A123456789123456789|BBBBBBBBBBB|CCCCCCCCCCCCC|DDDDDDDDDDDD|EEEEEEEEEEEEE|FFFFFFFFFFFFFFF"));

		//Print(AuDialog.ShowList("One|Two|Three|100 Four\r\naa\rbb\ncc"));
		//Print(AuDialog.ShowList(5));
		//Print(AuDialog.ShowList("One|Two|Three|100 Four", "Simple"));
		//var a = new string[100]; for(int i = 0; i < a.Length; i++) a[i] = (i+1).ToString();
		//Print(AuDialog.ShowList(a, "Simple"));

		//Print(AuDialog.ShowOKCancel("Text1"));
		//Print(AuDialog.ShowOKCancel("Text1", "Text2.", DIcon.Info, DFlags.Wider));
		//Print(AuDialog.ShowYesNo("Text1", "Text2.", DIcon.Info, DFlags.Wider));
		//AuDialog.ShowInfo("Text1", "Text2.", DFlags.Wider);
		//AuDialog.ShowWarning("Text1", "Text2.", DFlags.Wider);
		//AuDialog.ShowError("Text1", "Text2.", DFlags.Wider);

		//Print(AuDialog.ShowTextInputEx("Text1", "Text2."));
		//Print(AuDialog.ShowTextInput(out string s, "Simple", "aaaa", DEdit.Combo, new string[] { "text", "A", "B", "C" }));
		//Print(s);
...
i=mes(F"<>{_error.description}{_s}" "Test - error" "!")
	...
	//	case IEnumerable<string> a: Print("IEnumerable<string>", a); break;
	...
	//	case null: Print("null"); break;
	//	case int i: Print("int", i); break;
	//	case double d: Print("double", d); break;
	//		//default: throw new ArgumentException(); break; //don't need this, it's impossible
	//	}
	//}
	...
	static void TestValueTypes(Types<int, double> x)
	{
		...
		case 1: Print("int", x.v1); break;
		case 2: Print("double", x.v2); break;
		default: throw new ArgumentException(); break; //don't need this, it's impossible
		}
	}
	...
	{
		Print(a1.type, a1.v1, a1.v2);
		var t2 = a2.GetValueOrDefault();
		Print(t2.type, t2.v1, t2.v2);
		Print(a3.type, a3.v1, a3.v2);
		var t4 = a4.GetValueOrDefault();
		Print(t4.type, t4.v1, t4.v2);
	}
	...
	static void TestWndPropErrors()
	{
		...
		//w.Prop.Set("sett", 5);
		w.Prop.Remove("sett");
		Print(Native.GetError(), Native.GetErrorMessage());
		//Print(p);
		Print(w.Prop["sett"]);
		//Print(a.Count);
		//Print(a);
	...
	static void TestWinErrorMessageAndExceptions()
	{
		...
		try {
			...
			Print(e.NativeErrorCode, e.Message);
		}
	}
	...
	//	AuDialog.ShowTextInput(out string s, "td", editText: "AuDialog");
	...
	//{
	//	int f = AuDialog.ShowList("1 default|2 allDesktops|3 likeAltTab|4 retryFromTop|5 skipMinimized");
	//	if(f == 0) return;
	...
	static void TestWndAgain()
	{
		...
		//var w = Wnd.Find("Find");
		//w.AllChildren(false, true).ForEach(c => { Print(c); /*c.Show(false);*/ c.Focus(); 0.5.s(); });
		...
		//	Print("---");
		//});
		//Wnd.Misc.MainWindows().ForEach(t => { Print(t); t.Activate(); 0.5.s(); });
		...
		//AuDialog.Show("1");
		//AuDialog.Show("2");
		//AuDialog.ShowEx("2", timeoutS:30);
		//AuDialog.ShowEx("2", buttons:"OK|Cancel", timeoutS:30);
		//TaskDialogAPI(default(Wnd), default, "t", "m", null, 0, null, out int b);

		//var d = new AuDialog("aa", null, "1 Test", timeoutS: 15);
		//d.ButtonClicked += e =>
		//{
		//	e.DoNotCloseDialog = true;
		...
		//if(AuDialog.ShowTextInput(out string s)) Print(s);
		//var d = new AuDialog(text2: "small");
		//Print(d.ShowDialog());
		...
		//Print("self", w);
		//Print("parent", w.WndDirectParent);
		//Print("par or own", w.WndDirectParentOrOwner);
		//Print("owner", w.WndOwner);
		//Print("first child", w.WndFirstChild);
		//Print("last child", w.WndLastChild);
		//Print("child 2", w.WndChild(2));
		//Print("first sib", w.WndFirstSibling);
		//Print("last sib", w.WndLastSibling);
		//Print("next", w.WndNext);
		//Print("prev", w.WndPrev);
		//Print("window", w.WndWindow);
		...
		//Print(Process_.CurrentProcessHandle, Process_.CurrentThreadHandle, Process_.CurrentProcessId, Process_.CurrentThreadId);
	...
	{
		//var h = Api.CreateEvent(default, false, false, null);
	...
	{
		var hh = Api.SetWindowsHookEx(Api.WH_KEYBOARD_LL, s_testHookProc, default, 0);
		//AuDialog.Show();
		//Thread.Sleep(10000);
	...
	public static async Task<DResult> ShowAsync(string text1)
	{
		var td = new AuDialog(text1);
	...
	static void TestTaskDialogAsyncAwait()
	{
		...
		var td = AuDialog.ShowNoWaitEx("Another example", "text", "1 OK|2 Cancel", y: -1, secondsTimeout: 30);
		...
		//Thread.CurrentThread.Abort();
		//AuDialog.ShowEx("", "Text <a href=\"example\">link</a>.", onLinkClick: e =>
		//{
		//	Print(e.LinkHref);
		//	Thread.CurrentThread.Abort();
		//}
		...
		//AuDialog.Show("Simple example");
		//var td=AuDialog.ShowNoWait(null, "Simple example");
		//var td = AuDialog.ShowNoWaitEx(null, "Simple example", timeoutS: 3);
		//var td=AuDialog.ShowNoWait(null, "Simple example");
		//Print(1);
		//td.ThreadWaitClosed();
		//Print(2);
		...
		//	//MessageBox.Show("dd");
		//	//AuDialog.Show("dd");
		//	AuDialog.ShowEx("dd", timeoutS: 15);
		//	//Thread.CurrentThread.Abort();
		//	//0.1.s();
		...
		//var td = AuDialog.ShowNoWaitEx(e => { Print(e); }, "Another example", buttons: "1 OK|2 Cancel", y: -1, timeoutS: 30);
		//2.s(); //do something while the dialog is open in other thread
		//td.ThreadWaitClosed(); //wait until dialog closed (optional, just an example)

		//return;

		//Task.Run(() => AuDialog.Show("example"));
		//Task.Run(() => AuDialog.ShowEx("example", timeoutS: 15));
		//Print(1);
		//3.s(); //do something in this thread
		//Print(2);

		//var t = Task.Run(() => AuDialog.ShowEx("example", "<a href=\"example\">link</a>", buttons: "Yes|No|CL\nnn", icon: DIcon.App, flags: DFlags.CommandLinks, timeoutS: 15, onLinkClick: e => Print(e.LinkHref)));
		//var t = Task.Run(() => TaskDialogAPI(default(Wnd), default, "ti", "s1", "s2", 0, null, out int b));
		//Print(1);
		//3.s();
		//Print(2);
		//t.Wait();
		...
		////f.Click += (unu, sed) =>
		////{
		////	var r = AuDialog.Show("sync");
		////	var r = AuDialog.Show("sync", owner:f);
		////	Print(r);
		////};
		//f.Click += async (unu, sed) =>
		//{
		//	Print(Api.GetCurrentThreadId());
		//	//var r = await Task.Run(() => AuDialog.Show("async"));
		//	var r = await Task.Run(() => AuDialog.Show("async", owner: f));
		//	Print(r);
		//	Print(Api.GetCurrentThreadId());
		//};
		//f.ShowDialog();
...
@"Q:\app\dialog.ico",
	...
	static void TestFileOp()
	{
		...
		//Print(k.VolumeSerialNumber, k.FileIndex);

		//if(!Files.GetFileId(@"//Q7c/q/test", out var k2)) throw new AuException(0, "failed");
		//Print(k2.VolumeSerialNumber, k2.FileIndex);
		//Print(k2 == k);
		//Print(k.Equals(k2));
	...
	static void TestEnumDirectory()
	{
		...
		foreach(var f in Files.EnumDirectory(dir, fl)) {
			...
			Print(f.FullPath, f.Attributes.ToString("X"), f.Size, f.LastWriteTimeUtc.ToLocalTime(), f.Level);
	...
	static void TestDirectoryIsEmpty()
	{
		...
		s2 = @"\\?\D:\Test\x";
		s2 = @"D:\Test\protected";
		Print(PathIsDirectoryEmpty(s), PathIsDirectoryEmpty(s2));
		Print(IsEmptyDirectory(s), IsEmptyDirectory(s2));
		Perf.SpinCPU(100);
	...
	static void TestFileProperties()
	{
		...
		//Print(x.Attributes.ToString("X"), x.Size, x.LastWriteTimeUtc, x.CreationTimeUtc, x.LastAccessTimeUtc);
		...
		foreach(var f in Files.EnumDirectory(@"C:", FEFlags.AndSubdirectories | FEFlags.IgnoreAccessDeniedErrors)) {
			...
			//	Print(x.Attributes.ToString("X"), x.Size, x.LastWriteTimeUtc, x.CreationTimeUtc, x.LastAccessTimeUtc, k);
			//} else {
			//	var es = Native.GetErrorMessage();
			//	Print("FAILED", k, Files.ExistsAsAny(k), f.FullPath.Length, es);
			//}
		}
	...
	static void TestCalculateDriveSize()
	{
		...
		//	Print(Files.CalculateDirectorySize(f.FullPath) / 1024 / 1024, f);
		//}
	...
	static void TestExistsAsProtected()
	{
		...
		if(Files.GetProperties(s, out var p)) {
			Print(p.Size, p.Attributes);
			Print(p.CreationTimeUtc, p.LastWriteTimeUtc, p.LastAccessTimeUtc);
		}
	...
	static unsafe void TestPidl()
	{
		...
		//s = ":: 14001F706806EE260AA0D7449371BEB064C98683";
		Shell.Run(s);
		//Shell.Run(@"Q:\app\qm.exe", flags:SRFlags.SupportExtVerbs, verb:"properties"); 0.1.s();
		//AuDialog.Show();
	...
	static void TestVersion()
	{
		...
		//PrintHex(Ver.WinVer);
		//Print(Ver.MinWin8, Ver.MinWin8_1, Ver.MinWin10);
	...
	//	//var a4 = new Action(() => { var a = Api.VirtualAlloc(default, n1, 0x3000, 4); Api.VirtualFree(a); });
	//	//Perf.ExecuteMulti(5, 1000, a1, a2, a3, a4);
	...
	static void TestShellRun()
	{
		...
		//Shell.Run(@"Q:\my qm\test_run_dir.exe", more: new SRMoreParams() { WorkingDirectory=@"C:\Windows" });
		//Shell.Run(@"Q:\");
	...
	static unsafe void TestMain()
	{
		...
		catch(AuException ex) { Print(ex.NativeErrorCode, ex); }
		catch(ArgumentException ex) { Print(ex); }
		catch(Win32Exception ex) { Print(ex.NativeErrorCode, ex); }
		catch(Exception ex) when(!(ex is ThreadAbortException)) { Print(ex.GetType(), ex); }
		//catch(Exception ex) { }
...
";
	...
	static unsafe void TestVariadicDllImport()
	{
		...
		Print(n, Native.GetErrorMessage()); //0, no error
	...
	class TestOutputForm :Form
	{
		...
		public TestOutputForm()
		{
			...
			//Print(_tb.DetectUrls, _tb.EnableAutoDragDrop, _tb.MaxLength, _tb.ScrollBars, _tb.WordWrap);
	...
	static void TestOutputServer()
	{
		...
		Task.Run(() => { 0.5.s(); _TesOutputServerSpeed(); });
		//Print("<>test before setting notifications");
		//Task.Run(() => { 1.s(); Print("test after"); 1.s(); Output.Clear(); 1.s(); Print("test after Clear"); });
	...
	static void TestThreadPoolPerf()
	{
		...
		Print(GetThreadCount()); //9
								 //for(int i = 0; i < 10; i++) ThreadPool.QueueUserWorkItem(sta => { 1.s(); }); //12 if 1, but 14 if more
		for(int i = 0; i < 2; i++) Task.Run(() => { 10.s(); }); //12 if 1, but 14 if more
		...
		//			Perf.First();
		//#if false
		//		Task.Run(() => { 1.s(); Perf.Write(); }); //12 threads, speed 6000/10 (4000 non-ngened)
		//#elif false
		...
		//			//Print(v1, v2, v3, v4);
		//			ThreadPool.QueueUserWorkItem(sta => { 1.s(); Perf.Write(); }); //12 threads, speed 400/8

		//			//ThreadPool.UnsafeQueueUserWorkItem(sta => { 1.s(); Perf.Write(); }, null); //12 threads, speed 400/8
		//#elif false
		//			var tim=new System.Threading.Timer() //difficult
		//#else
		//			var t = new Thread(() => { 1.s(); Perf.Write(); }); //9 threads, speed 400
		//			t.IsBackground = true;
		//			t.Start();
		//#endif
		//			Perf.Next();
	...
	static void TestMouseMoveClick()
	{
		...
		//f.MouseDown += (unu, sed) => { if(printMouse) Print("down", Time.Milliseconds, Mouse.XY); };
		//f.MouseUp += (unu, sed) => { if(printMouse) Print("up", Time.Milliseconds, Mouse.XY); };
		//f.ShowDialog();
		//0.2.s();
	...
	class FormTestMouse :Form
	{
		...
		{
			Print("down", Time.Milliseconds, Mouse.XY);
			base.OnMouseDown(e);
		}
		protected override void OnMouseUp(MouseEventArgs e)
		{
			Print("up", Time.Milliseconds, Mouse.XY);
			base.OnMouseDown(e);
		}
	...
	{
		var w = (Wnd)f;
		Print("start", Time.Milliseconds, Mouse.XY);
		Perf.First();
		...
		Print("end", Time.Milliseconds, Mouse.XY);
		//Perf.NW();
	}
	...
	static void TestDisableWarnings()
	{
		...
		using(Options.DisableWarnings("*.NET*")) {
			PrintWarning("Test.");
			PrintWarning("nondisabled.");
			PrintWarning("Failed to get .NET control names.");
		}
		Print("end");
		PrintWarning("nondisabled: Failed to get .NET control names.");
	}
	...
	{
		//Print(x.Type, y.Type, x.Value, y.Value, x.Value / 10000.0, y.Value / 10000.0);
		Print(x.Type, y.Type, x.Value, y.Value, x.FractionValue, y.FractionValue);
		if(x.Type == Coord.CoordType.Fraction) Print(3);
		if(y.Type == Coord.CoordType.Fraction) Print(3);
	}
	...
	{
		Print(x.Type, y.Type, z.Type, q.Type);
		if(x.Type == Coord.CoordType.Fraction) Print(3);
		if(y.Type == Coord.CoordType.Fraction) Print(3);
	...
	static void TestScreen_()
	{
		...
		AuDialog.Options.DefaultScreen = 2; //invalid
		AuDialog.Show();
	...
	static void TestMoveInScreenWithCoord()
	{
		...
		//AuDialog.Show();
		//AuDialog.ShowEx(x:200, y:100);
		//AuDialog.ShowEx(x:200, y:1400);
		//AuDialog.ShowEx(x:200, y:1700, flags:DFlags.RawXY);
		//AuDialog.ShowEx(y:1700, flags:DFlags.RawXY);
		//AuDialog.ShowEx(x: Coord.Reverse(-10), y: Coord.Reverse(-10));
		//AuDialog.ShowEx(x: Coord.Fraction(0.1), y: Coord.Fraction(0.95));
		//AuDialog.ShowEx(x: Coord.Fraction(0.1), y: Coord.Fraction(1.5));
		//AuDialog.ShowEx(x: Coord.Fraction(0.1), y: Coord.Fraction(1.5), flags:DFlags.RawXY);

		//AuDialog.ShowEx(x: Coord.Reverse(0), y: Coord.Reverse(0));
		//AuDialog.ShowEx(x: Coord.Right, y: Coord.Bottom);
		AuDialog.ShowEx(x: Coord.Max, y: Coord.Max);

		//Shell.Run("notepad.exe");
		//AuDialog.Show();
		//AuDialog.ShowEx(x: 200, y: 100);
		//AuDialog.Show(owner:null);

		//var f = new Form();
		////f.Click+=(unu, sed) => AuDialog.Show("test");
		////f.Click+=(unu, sed) => AuDialog.Show("test", owner:f);
		////f.Click+=(unu, sed) => AuDialog.Show("test", owner:f.Wnd_());
		//AuDialog.Options.AutoOwnerWindow = true;
		//f.Click+=(unu, sed) => AuDialog.Show("test");
		//f.ShowDialog();
	}
	...
	static void TestRejectTypes()
	{
		...
		//Print(AuDialog.Show(buttons: "One|Two"));
		//Print(AuDialog.ShowList("One|Two"));
		//Print(AuDialog.ShowList(new string[] { "One", "Two" }));
		//Print(AuDialog.ShowList(new List<string> { "One", "Two" }));
		//Print(AuDialog.ShowList(5));
		...
		//		AuDialog.ShowEx("test", timeoutS: 1);
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	static AuDialog _TestRejectTypes_1()
	{
	...
	static void TestApiTimeBeginPeriodBug()
	{
		...
		//	using(new Time.LibSleepPrecision(1)) {
		//		Print(v, Time.LibSleepPrecision.Current);
		//	}
		//	0.25.s();
		//}
		//}
	...
	static void TestThreadStaticDtor()
	{
		...
		//Print("main", Api.GetCurrentThreadId());

		for(int i = 0; i < 3; i++) {
			var t = new Thread(() =>
			{
				//Print(i+1, Api.GetCurrentThreadId());
				_TestThreadStaticDtor.Create();
				//t_loc.Value = null;
	...
	static unsafe void TestLibBuffers()
	{
		...
		//Print(x.TargetPath, x.Arguments, x.Description, x.WorkingDirectory);
	...
	public static unsafe class TestFormat
	{
		...
		static void TestFormatSpeed()
		{
			...
			for(int i1 = 0; i1 < 9; i1++) {
				...
				Perf.Next();
				for(int i2 = 0; i2 < n2; i2++) { fixed (char* p = ca) { wsprintfW(p, "%i    %i    %s", __arglist(x, y, s)); r2 = new string(ca, 0, Au.Util.LibCharPtr.Length(p)); } }
				Perf.Next();
	...
	static unsafe LPARAM _TestRecordMouseMove_Proc(int code, LPARAM msg, LPARAM lParam)
	{
		...
		//Print(m.pt.X, m.pt.Y);
		//Print(m.pt.X - s_pp.X, m.pt.Y - s_pp.Y);
		...
		return Api.CallNextHookEx(default, code, msg, lParam);
	}
