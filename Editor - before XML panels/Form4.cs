...
namespace Editor
{
	public partial class Form4 :Form
	{
		...
		public Form4()
		{
			...
			//treeview
			_tvPane = new CatTreePane(_docker, DockStyle.Left);
...
#else
#endif
			//_tvPane.Size = new Size(tvWidth, z.Height);
			_tvPane.Size = new Size(200, 0);

			//open
			_openPane = new CatOpenPane(_docker, DockStyle.Right);
			_openPane.Text = _openPane.Name = "Open";
			_openPane.Size = new Size(100, 0);

			//output
			_outputPane = new CatOutputPane(_docker, DockStyle.Bottom);
			_outputPane.Text = _outputPane.Name = "Output";
			_outputPane.Size = new Size(0, 150);

#if MORE_PANES
			//find
			_findPane = new CatFindPane(_docker, DockStyle.Bottom);
			_findPane.Text = _findPane.Name = "Find";
			_findPane.Size = new Size(100, 100);

			//_openPane.DPCanDockOnDockPanels = true;

			//running
			_runningPane = new CatRunningPane(_docker, DockStyle.Bottom);
			_runningPane.Text = _runningPane.Name = "Running";
			_runningPane.Size = new Size(100, 100);

			////recent
			//_recentPane = new CatRecentPane();
			//_recentPane.Text = _recentPane.Name = "Recent";
			//_recentPane.Size = new Size(100, 100);
#endif

			//status bar
			_statusBar = new Scintilla();
			_statusBar.Name = nameof(_statusBar);
			_statusBar.Size = new Size(z.Width, 40);
			_statusBar.Dock = DockStyle.Bottom;

			//strips
			_strips = new Panel();
			_strips.Name = nameof(_strips);
			_strips.Size = new Size(z.Width, 50);
			...
			_openPane.AddToParent(contentPanel);
			_outputPane.AddToParent(contentPanel);
#if MORE_PANES
			_findPane.AddToParent(contentPanel);
			_runningPane.AddToParent(contentPanel);
#endif
			_tvPane.AddToParent(contentPanel);
		...
		public class CatTreePane :GPanel
		{
#if TV_NATIVE
			Wnd _w, _wPane;

			public CatTreePane(GPanels docker, DockStyle dockStyle) : base(docker, dockStyle)
			{
			}
		...
		public class CatOutputPane :GPanel
		{
			public CatOutputPane(GPanels docker, DockStyle dockStyle) : base(docker, dockStyle)
			{
				var c = new Scintilla();
		...
		public class CatFindPane :GPanel
		{
			public CatFindPane(GPanels docker, DockStyle dockStyle) : base(docker, dockStyle)
			{
				var c = new ListView();
				c.Dock = DockStyle.Fill;
				this.Controls.Add(c);
		...
		public class CatOpenPane :GPanel
		{
			public CatOpenPane(GPanels docker, DockStyle dockStyle) : base(docker, dockStyle)
			{
				var c = new ListView();
				c.Dock = DockStyle.Fill;
				this.Controls.Add(c);
		...
		public class CatRunningPane :GPanel
		{
			public CatRunningPane(GPanels docker, DockStyle dockStyle) : base(docker, dockStyle)
			{
				var c = new ListView();
				c.Dock = DockStyle.Fill;
				this.Controls.Add(c);
		...
		public class CatRecentPane :GPanel
		{
			public CatRecentPane(GPanels docker, DockStyle dockStyle) : base(docker, dockStyle)
			{
				var c = new ListView();
				c.Dock = DockStyle.Fill;
				this.Controls.Add(c);
