...
namespace Au.Tools
{
	class DAWnd : KDialogWindow
	{
		...
		public DAWnd(AWnd wnd = default, bool uncheckControl = false) {
			Title = "Find window or control";

			var b = new AWpfBuilder(this).WinSize((600, 440..), (600, 460..)).Columns(-1, 0);
			...
			AWnd.More.SavedRect.Restore(this, App.Settings.tools_AWnd_wndPos, o => App.Settings.tools_AWnd_wndPos = o);
		}
...
#region capture
		...
		void _cCapture_CheckedChanged() {
			_capt ??= new TUtil.CaptureWindowEtcWithHotkey(_cCapture, _Capture, () => AWnd.FromMouse().Rect);
...
#region OK, Test
		...
		private void _bTest_Click(WBButtonClickArgs ea) {
			...
			TUtil.RunTestFindObject(code, wndVar, _wnd, _bTest, _speed, o => {
				var w = (AWnd)o;
				var r = w.Rect;
...
#region tree
		...
		void _FillTree() {
			...
			void _AddChildren(AWnd wParent, _TreeItem nParent) {
			...
			_tree.SetItems(new _TreeItem[1] { xWindow }, false);

			//AOutput.Write(xWindow.Descendants().Select(o=>o.c));
...
@"An accessible object in the window. Format: a 'role' name.
...
If unchecked, returns default(AWnd).");
...
@"This dialog creates code to <help AWnd.Find>find window<> or <help AWnd.Child>control<>.
