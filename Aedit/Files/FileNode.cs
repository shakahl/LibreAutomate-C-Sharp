...
partial class FileNode : ATreeBase<FileNode>, ITreeViewItem
{
	#region types
	...
	public static string CreateNameUniqueInFolder(FileNode folder, string fromName, bool forFolder, bool autoGenerated = false) {
		...
		for (int i = 2; ; i++) {
			...
			if (!_Exists(s)) {
				if(!autoGenerated) AOutput.Write($"Info: name \"{oldName}\" has been changed to \"{s}\", to make it unique in the folder.");
				return s;
			}
		}
