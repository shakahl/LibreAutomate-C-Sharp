...
partial class FOptions : DialogForm
{
	...
	void _ApplyGeneral()
	{
		...
		App.Settings.runHidden = _cRunHidden.Checked;

		if(_eStartupScripts.Modified) App.Model.StartupScriptsCsv = _eStartupScripts.Text;
	...
	private void _startupScripts_Validating(object sender, CancelEventArgs e)
	{
		...
		try {
			...
			foreach(var v in t.Data) {
				...
				if(App.Model.FindScript(script) == null) { err = "Script not found: " + script; break; }
...
", icon: DIcon.Info, onLinkClick: e => { AFile.SelectInExplorer(e.LinkHref); });
	...
	{
		_cComplParenSpace.Checked = App.Settings.ci_complParenSpace;
		_cStringEnterRN.Checked = 0 == App.Settings.ci_correctStringEnter;
	}

	void _ApplyCode()
	{
		App.Settings.ci_complParenSpace = _cComplParenSpace.Checked;
		App.Settings.ci_correctStringEnter = (byte)(_cStringEnterRN.Checked ? 0 : 1);
	}
