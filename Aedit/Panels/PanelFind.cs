...
class PanelFind : AuUserControlBase
{
	...
	void _ShowRegexInfo(_SciTextBox tb, bool F1) {
		...
		if (!_regexWindow.Window.IsHandleCreated) {
			...
			r.Offset(0, -20);
			//_regexWindow.Show(App.Wnd, r, true); //TODO
	...
	private void _bOptions_Click(object sender, EventArgs e) {
		...
		App.Settings.find_searchIn = _searchIn = f._cbSearchIn.SelectedIndex;
		App.Settings.find_skip = f._tSkip.Text; _aSkipWildcards = null;
	}
	...
	public void ZCtrlF(Control c) {
		//if (!Visible) Panels.PanelManager.ZGetPanel(this).Visible = true; //TODO
		string s = "";
		switch (c) {
		case AuScintilla k:
			s = k.Z.SelectedText();
