		...
		protected class Floaty
		{
			...
			internal void MakeFloating(bool startMove)
			{
				if(!_isFloating) {
					...
					_dockState.dockStyle = _panel.Dock;
			...
			internal void MakeDocked(bool dropped)
			{
				...
				_panel.Dock =m==null ?_dockState.dockStyle :DockStyle.None;
				_panel.Bounds = _dockState.bounds;

				((Wnd)_panel).ZorderAfter(_FindToInsertAfter(dropped));
				if(m != null) m.OnChildPanelDocked(_panel);
				_panel.Visible = true;
