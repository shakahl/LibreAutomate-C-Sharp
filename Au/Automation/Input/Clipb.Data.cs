...
namespace Au
{
	public static partial class Clipb
	{
		...
		/// </summary>
		/// <remarks>
		/// The <b>AddX</b> functions add data to the variable (not to the clipboard). Then <see cref="SetClipboard"/> copies the added data to the clipboard. Also you can use the variable with <see cref="Clipb.PasteData"/>.
		...
		public class Data
		{
			...
			Data _Add(object data, int format, bool minimalCheckFormat = false)
			{
			...
			public Data AddText(string text, int format = ClipFormats.Text)
			{
			...
			public Data AddBinary(byte[] data, int format)
			{
			...
			/// <exception cref="ArgumentNullException"></exception>
			public Data AddImage(Image image)
			{
			...
			/// ]]></code>
			/// </example>
			public Data AddHtml(string html)
			{
			...
			/// <exception cref="ArgumentNullException"></exception>
			public Data AddFiles(params string[] files)
			{
...
namespace Au.Types
{
	...
	/// These and other standard and registered format ids can be used with <see cref="Clipb.Data"/> class functions.
	/// </summary>
	public static class ClipFormats
	{
		/// <summary>The text format. Standard, API constant CF_UNICODETEXT. The default format of <see cref="Clipb.Data"/> add/get text functions.</summary>
		public const int Text = Api.CF_UNICODETEXT;

		/// <summary>The image format. Standard, API constant CF_BITMAP. Used by <see cref="Clipb.Data"/> add/get image functions.</summary>
		public const int Image = Api.CF_BITMAP;

		/// <summary>The file-list format. Standard, API constant CF_HDROP. Used by <see cref="Clipb.Data"/> add/get files functions.</summary>
		public const int Files = Api.CF_HDROP;

		/// <summary>The HTML format. Registered, name "HTML Format". Used by <see cref="Clipb.Data"/> add/get HTML functions.</summary>
		public static int Html { get; } = Api.RegisterClipboardFormat("HTML Format");

		/// <summary>The RTF format. Registered, name "Rich Text Format". Used by <see cref="Clipb.Data"/> add/get RTF functions.</summary>
		...
		/// <param name="textEncoding">Text encoding, if it's a text format. Used by <see cref="Clipb.Data.GetText"/>, <see cref="Clipb.Data.AddText"/> and functions that call them. For example <see cref="Encoding.UTF8"/> or <see cref="Encoding.Default"/> (ANSI). If null, text of unknown formats is considered Unicode UTF-16 (no encoding/decoding needed).</param>
