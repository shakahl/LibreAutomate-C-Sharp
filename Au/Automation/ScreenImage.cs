...
namespace Au
{
	...
	public static class ScreenImage
	{
		...
		/// </remarks>
		/// <exception cref="AuException">Failed. For example hbitmap is Zero.</exception>
		...
		/// Returns null if not found. To throw exception you can use operator +: <c>var r = +ScreenImage.Find(...);</c>
		/// </summary>
		/// <param name="image">
		...
		/// IEnumerable of string, Bitmap, int/ColorInt/Color or object - multiple images or colors. Default action - find any. If flag AllMustExist - must find all.
		/// </param>
		/// <param name="area">
...
#pragma warning restore 649
			...
			void _GetAreaPixels(RECT r)
			{
				...
				Api.ReleaseDC(w, dc);
				if(!bbOK) throw new AuException("BitBlt"); //fails only if a hdc is invalid
															//_Debug("captured to MemBmp");
															//get pixels
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
	...
	public class SIResult
	{
		...
		/// Alternatively you can use <see cref="ExtensionMethods.OrThrow(SIResult)"/>. Examples are there.
		/// </summary>
		/// <exception cref="NotFoundException">x is null.</exception>
		public static SIResult operator +(SIResult x) => x ?? throw new NotFoundException("Not found (image on screen).");
		...
		internal void LibMouseAction(MButton button, Coord x, Coord y)
		{
			...
			if(area.Type == SIArea.AType.Screen) {
				...
				if(area.Type == SIArea.AType.Acc) {
					if(!area.A.GetRect(out var r, area.W)) throw new AuException(0, "*get rectangle");
