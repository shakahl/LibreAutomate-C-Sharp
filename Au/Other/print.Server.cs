...
namespace Au
{
	public static partial class print
	{
		...
		/// 1. When your application starts, create a <b>print.Server</b> instance and assign to a static variable. Call <see cref="Start"/>.
		...
		public unsafe class Server
		{
			...
			public bool Start() {
				lock (this) {
					...
					s_localServer = this; //if global server, will work as local when writes same process
					_isStarted = true;
				}
				return true;
			}
		...
		unsafe class _ClientOfGlobalServer
		{
			...
			bool _Connect() {
				...
				if (_mailslot.Is0) {
					...
					_timer = WaitableTimer.Open(Server.TimerName_, noException: true);
					if (_timer == null) {
						_mailslot.Dispose();
						return false;
					}
...
namespace Au.Types
{
	...
	/// More info: <see cref="print.Server"/>, <see cref="print.Server.GetMessage"/>.
	/// </summary>
