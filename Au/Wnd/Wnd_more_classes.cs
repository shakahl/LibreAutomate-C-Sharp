	...
	public partial struct Wnd
	{
		public static partial class More
		{
			...
			public sealed class WinFormsControlNames :IDisposable
			{
				...
				/// <exception cref="AException">Failed to allocate process memory (see <see cref="AProcess.Memory"/>) needed to get control names, usually because of [](xref:uac).</exception>
				public WinFormsControlNames(Wnd w)
				{
					_pm = new AProcess.Memory(w, 4096); //throws
					_w = w;
				}
