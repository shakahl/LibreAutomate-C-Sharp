namespace Au.More
{
	...
	/// 		if (WndSavedRect.FromString(Registry.GetValue(c_rkey, c_rvalue, null) as string, out var v)) {
	/// 			this.Bounds = v.NormalizeRect();
	/// 			if(v.Maximize) this.WindowState = FormWindowState.Maximized;
	/// 		}
	/// 		//the same:
	/// 		//WndSavedRect.Restore(this, Registry.GetValue(c_rkey, c_rvalue, null) as string);
	/// 	}
	/// 
	/// 	protected override void OnFormClosing(FormClosingEventArgs e) {
	/// 		Registry.SetValue(c_rkey, c_rvalue, new WndSavedRect(this).ToString());
	...
	/// 	
	/// 	public Form8() {
	/// 		WndSavedRect.Restore(this, Registry.GetValue(c_rkey, c_rvalue, null) as string, s1 => Registry.SetValue(c_rkey, c_rvalue, s1.ToString()));
	/// 	}
	/// }
	/// ]]></code>
	...
	public struct WndSavedRect
	{
		...
		//public WndSavedRect(Form form) { //rejected
