...
namespace Au
{
	...
	/// </summary>
	public sealed class Uac : IDisposable
	{
		...
		/// <summary>
		/// Gets the <see cref="Uac">UAC</see> elevation type of the process.
		/// </summary>
		...
		/// <summary>
		/// Returns true if the process has <see cref="Uac">uiAccess</see> property.
...
#pragma warning restore 649
		...
		/// <summary>
		/// Gets the <see cref="Uac">UAC</see> integrity level (IL) of the process.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		/// Opens process access token and creates/returns new <see cref="Uac"/> variable that holds it. Then you can use its properties.
		/// Returns null if failed. For example fails for services and some other processes if current process is not administrator.
		/// To get <b>Uac</b> of this process, use <see cref="OfThisProcess"/>.
		/// </summary>
		...
		/// <summary>
		/// Gets <see cref="Uac"/> variable for this process.
		/// </summary>
		public static Uac OfThisProcess {
			get {
		...
		static Uac _thisProcess;

		/// <summary>
		/// Returns true if this process is running as administrator, ie if the user belongs to the local Administrators group and the process is not limited by <see cref="Uac">UAC</see>.
		...
		*/
		...
		/// <summary>
		/// Returns true if <see cref="Uac">UAC</see> is disabled (turned off) completely (not just disabled UAC consent screeen/dialog).
		/// </summary>
		...
		{
			//if(Ver.MinWin8) return false; //UAC cannot be disabled so easily, but can
			Uac x = OfThisProcess;
			switch(x.Elevation) {
...
namespace Au.Types
{
	/// <summary>
	/// UAC integrity level.
	/// See <see cref="Uac.IntegrityLevel"/>.
	/// </summary>
	public enum UacIL
	{
