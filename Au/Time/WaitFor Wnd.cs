...
namespace Au
{
	...
	/// </summary>
	/// <remarks>
	/// All functions have a <i>secondsTimeout</i> parameter. It is the maximal time to wait, seconds. If it is 0, waits indefinitely. If &gt;0, after that time interval throws <see cref="TimeoutException"/>. If &lt;0, after that time interval stops waiting and returns default value (false, default(Wnd), etc).
	/// 
	...
	public static partial class WaitFor
	{
		...
		/// The maximal time to wait, seconds. If 0, waits indefinitely. If &gt;0, after that time interval throws <see cref="TimeoutException"/>. If &lt;0, after that time interval returns default(Wnd).
		/// </param>
		...
		public static Wnd WindowExists(double secondsTimeout = 0.0,
			...
			WFFlags flags = 0, Func<Wnd, bool> also = null, object contains = null, bool not = false)
		{
			var f = new Wnd.Finder(name, className, programEtc, flags, also, contains);
			return WindowExists(f, secondsTimeout, not);
		}

		/// <summary>
		/// The same as <see cref="WindowExists(double, string, string, string, WFFlags, Func{Wnd, bool}, object, bool)"/>, just arguments are passed differently.
		/// </summary>
		...
		///
		/// More examples: see <see cref="WindowExists(double, string, string, string, WFFlags, Func{Wnd, bool}, object, bool)"/>.
		/// </remarks>
		/// <example>
		/// <code><![CDATA[
		...
		//TODO: need Wnd.Finder[]. It would wait for one of windows and returns 1-based index or 0. The same for WindowExists.
		//	public static int WindowActive(double secondsTimeout, bool not, params Wnd.Finder[] f)

		/// <summary>
		/// The same as <see cref="WindowActive(double, string, string, string, WFFlags, Func{Wnd, bool}, object, bool)"/>, just arguments are passed differently.
		/// </summary>
		/// <param name="f">Window properties etc.</param>
		/// <param name="secondsTimeout"></param>
		/// <param name="not"></param>
		...
		/// For 'exists and is visible' use <see cref="WindowExists(double, string, string, string, WFFlags, Func{Wnd, bool}, object, bool)"/>. With default flags it waits for visible window and ignores invisible windows.
		/// </summary>
		/// <param name="w">A window or control.</param>
		/// <param name="secondsTimeout">
		/// The maximal time to wait, seconds. If 0, waits indefinitely. If &gt;0, after that time interval throws <see cref="TimeoutException"/>. If &lt;0, after that time interval returns false.
