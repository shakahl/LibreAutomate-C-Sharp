...
namespace Au
{
	...
	/// ]]></code>
	/// </example>
	public unsafe partial class AMenu : MTBase
	{
		/// <summary>
		...
		public class MenuItem : MTItem
		{
			...
			internal MenuItem(AMenu m, bool isDisabled, bool isChecked = false) {
				_m = m;
			...
			/// </summary>
			/// <exception cref="InvalidOperationException">The 'set' function throws if the item isn't checkable. Use <see cref="AMenu.AddCheck"/> or <see cref="AMenu.AddRadio"/>.</exception>
		...
		static AMenu() {
			AWnd.More.RegisterWindowClass("AMenu", etc: new() { style = Api.CS_HREDRAW | Api.CS_VREDRAW | Api.CS_DROPSHADOW, mCursor = MCursor.Arrow });
		}
		...
		/// m.Show();
		/// 
		/// static void _Dir(AMenu m, DirectoryInfo dir) {
		...
		/// <example>
		/// <code><![CDATA[
		/// var m2 = new AMenu(); m2.AddCheck("C1"); m2.AddCheck("C2");
		...
		/// 
		/// Does not get items in submenus. Submenus are separate <b>AMenu</b> objects and you can use their <b>Items</b> property.
		/// </remarks>
		...
		public int Show(MSFlags flags = 0, POINT? xy = null, RECT? excludeRect = null, AnyWnd owner = default) {
			...
			try {
				...
				timer = new(t => { //close if mouse clicked a non-menu window or if activated another window
					...
					mouseState = ms;
					if (clicked) _CloseIfClickedNotMenu(AWnd.FromMouse());
					else if (AWnd.Active != wFore) Close();
				});
				timer.Every(30);

				static int _GetMouseState() =>
					(AKeys.UI.GetKeyState(KKey.MouseLeft) & 0x1)
					| ((AKeys.UI.GetKeyState(KKey.MouseRight) & 0x1) << 1)
					| ((AKeys.UI.GetKeyState(KKey.MouseMiddle) & 0x1) << 2)
					;
				...
				//note: in some cases toggled state may not change when clicked. Eg when clicked a taskbar button that activates another window. Then helps if (AWnd.Active!=wFore) Close();.

				void _CloseIfClickedNotMenu(AWnd w) {
					//if(!w.Get.Owners(andThisWindow: true).Contains(_w)) Close(); //no, user may want nested root menus, although it is rare
					if (w != _w && !w.ClassNameIs("AMenu")) Close();
				}
				...
				if (!foreground) {
					...
					if (!g.IsUp) {
						...
						//AOutput.Write(ok);
						if(Api.PeekMessage(out var v, _w, Api.WM_CHAR, Api.WM_CHAR, Api.PM_NOREMOVE)) AOutput.Write("peek", v);
					}
		...
		void _ShowSubmenu(MenuItem b, bool focusFirst = false, AMenu m = null) {
			...
			if (!contextMenu) {
				...
				if (b.clicked is Action<AMenu> menu) {
					m = _sourceFile == null ? new() : new AMenu(null, _sourceFile, b.sourceLine);
					base._CopyProps(m);
		...
		void _Show(MSFlags flags, POINT? xy, RECT? excludeRect, AWnd owner) { //CONSIDER: _Show(MSPosition pos = null) //then could precisely specify offsets etc
		...
		int _SubmenuTimer(MenuItem item = null) {
			...
			_sub.timer ??= new(t => {
				...
				} else if (_sub.child != null && !_sub.child._w.Rect.Contains(AMouse.XY)) {
					_sub.child.Close();
				}
			});
			int R = Api.SystemParametersInfo(Api.SPI_GETMENUSHOWDELAY, 400);
			_sub.timer.After(R, item);
			return R;
		}
		...
		void _WmMouseleave() {
			_mouse.track = false;
			if (_iHot < 0) return;
			if (_sub.child?._w.Rect.Contains(AMouse.XY) ?? false) return;
			_SetHotItem(-1);
			_SubmenuTimer();
		}
		...
		void _ContextMenu(MenuItem b) {
			...
			if (canEdit || canGo) {
				var m = new AMenu();
				if (canEdit) m["Edit menu item"] = _ => AScriptEditor.GoToEdit(_sourceFile, b.sourceLine);
		...
		bool _TopHot(out (AMenu m, int i, MenuItem b) r) {
			var m = this; while (m._sub.child != null) m = m._sub.child;
			int i = m._iHot;
		...
		void _WmChar(char c) { //called for top menu
			...
			for (int i = 0; i < _a.Count; i++) {
				...
				string s = b.Text;
				int j = AStringUtil.FindUnderlineChar(s);
				if (j < 0) continue;
				c = s[j];
				if (c == cu || c == cl) {
		...
		static bool _IsPassKey(KKey k)
			...
			or (>= KKey.F1 and <= KKey.F24) || AKeys.IsMod(KMod.Ctrl | KMod.Alt | KMod.Win);
		...
		/// </remarks>
		public Func<AMenu, HookData.Keyboard, MKHook> KeyboardHook { get; set; }
	}
}
...
namespace Au.Types
{
	...
	public enum MSFlags
	{
		...
		/// <summary>Underline characters preceded by &amp;, regardless of Windows settings. More info: <see cref="AStringUtil.RemoveUnderlineChar"/>.</summary>
		Underline = 0x4000000,
	...
	/// <summary>
	/// Used with <see cref="AMenu.KeyboardHook"/>.
	/// </summary>
	public enum MKHook
	{
