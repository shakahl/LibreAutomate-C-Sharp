...
//TODO: delete all except MTImage.
...
namespace Au.Types
{
	...
	public abstract class MTBase_old
	{
		...
		private protected MTBase_old(string sourceFile, int sourceLine)
		{
			_onClick = OnClick_;
		...
		// If not full path, searches in <see cref="AFolders.ThisAppImages"/>; see also <see cref="MTBase_old.IconFlags"/>.
		...
		public int IconSize {
			...
			set {
				...
				_iconSize = value;
				MainToolStrip.ImageScalingSize = new Size(value, value); //TODO: this is temporarily used for toolbars. Later remove when made PM-DPI-aware.
			}
		}
		int _iconSize = 16;

		/// <summary>
		/// Gets "invoke method" image from Au library resources. Can be used for <see cref="DefaultIcon"/> of toolbars.
		/// </summary>
		...
		//	Eg method SetCommonIcon(Image icon, int dpi) or property (List<(int dpi, Image image)> CommonIconForDpi.
		//	Also for CommonSubmenuIcon, DefaultIcon, DefaultSubmenuIcon.
		//	Also toolbars should be auto-scaled when DPI changed.
		//	Now, if IconSize=32, default images scaled and ugly.

		/// <summary>
		/// Gets "drop-down menu" image from Au library resources. Can be used for <see cref="DefaultSubmenuIcon"/> of toolbars.
		/// </summary>
		...
		private protected void _SetItemProp(bool isTB, bool isSub, ToolStripItem item, Action<MTClickArgs> onClick, MTImage icon, int sourceLine)
		{
			...
			if(onClick != null) {
				...
				//APerf.First();
				if(iconObj == null && ExtractIconPathFromCode) iconObj = AIcon.IconPathFromCode_(onClick.Method);
		...
		void _SetItemFileIcon(bool isTB, ToolStripItem item, string s)
		{
			...
			//Reserve space for image. If toolbar, need to do it for each button, else only for the first item.
			item.Image = t_imagePlaceholder ??= new Bitmap(1, 1);
			//perf.NW();
		}
		[ThreadStatic] static Image t_imagePlaceholder;
		...
		internal void GetIconsAsync_(ToolStrip ts, List<IconsAsync_.Item> list = null)
		{
			...
			//AOutput.Write(IconSize, ts.ImageScalingSize.Width);
			//_AsyncIcons.GetAllAsync(_AsyncCallback, ts.ImageScalingSize.Width, 0 /*IconFlags*/, ts);
			//_AsyncIcons.GetAllAsync(_AsyncCallback,AKeys.IsScrollLock ? IconSize : ts.ImageScalingSize.Width, 0 /*IconFlags*/, ts);
			_AsyncIcons.GetAllAsync(_AsyncCallback, IconSize, 0 /*IconFlags*/, ts);
			//TODO: cannot use IconSize because of control bug: if primary screen DPI 150%, in other screen ignores IconSize=16 and shrinks icons to 16.
		}
	...
	/// <summary>
	/// Used with <see cref="MTBase_old.ActionThread"/>.
	/// </summary>
	...
	/// <summary>
	/// Used with <see cref="MTBase_old.ExceptionHandling"/>.
	/// </summary>
	public enum MTExcept : byte
	{
		/// <summary>
		/// Don't handle exceptions. This is default.
		/// However if <see cref="MTBase_old.ActionThread"/> is not <see cref="MTThread.Current"/>, handles exceptions and shows warning.
	...
	/// - default - no icon. If <see cref="MTBase_old.ExtractIconPathFromCode"/> == true, extracts icon path from <i>onClick</i> code like <c>AFile.TryRun(@"c:\path\file.exe")</c> or <c>AFile.TryRun(AFolders.System + "file.exe")</c>.
	/// - "" - no icon.
	/// </remarks>
	[DebuggerStepThrough]
