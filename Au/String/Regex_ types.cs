...
namespace Au.Types
{
	...
	{
		internal RXMatch(Regex_ regex, string subject, int rc, in Cpp.RegexMatch k)
		{
		...
		/// <summary>
		/// Gets the return value of the <see cref="Regex_.Match"/> call.
		...
		/// If more than 1 group have this name, prefers the first group that matched (<see cref="RXGroup.Exists"/> is true).
		/// </remarks>
		/// <seealso cref="Regex_.GroupNumberFromName"/>
		...
		//Used by Regex_.ReplaceAll_ to avoid repl.Substring.
	...
	/// Used with <see cref="RXMatch"/>, <see cref="Regex_"/> and some String extension methods.
	/// </summary>
	/// <remarks>
	...
	/// 
	/// When a group matches multiple times, the RXGroup variable contains only the last instance. For example, if subject is <c>"begin 12 345 67 end"</c> and regular expression is <c>(\d+ )+</c>, value of group 1 is <c>"67"</c>. If you need all instances (<c>"12"</c>, <c>"345"</c>, <c>"67"</c>), instead use .NET <see cref="Regex"/> and <see cref="Group.Captures"/>. Also you can get all instances with <see cref="Regex_.Callout"/>.
	/// 
	/// Examples and more info: <see cref="RXMatch"/>, <see cref="Regex_"/>.
	/// </remarks>
	...
	/// <summary>
	/// Rarely used parameters for <see cref="Regex_"/> class functions.
	/// </summary>
	/// <remarks>
	...
	/// See <see cref="Regex_.Callout"/>.
	/// </summary>
	public delegate void RXCalloutFunc(RXCalloutData b);

	/// <summary>
	/// Managed version of PCRE API struct pcre2_callout_block.
	/// When you set <see cref="Regex_.Callout"/>, your callout function's parameter is of this type.
...
#pragma warning disable 1591 //no XML doc
	/// <summary>
	/// Flags for <see cref="Regex_"/> constructor.
	...
	public enum RXFlags :ulong
	{
		...
		//Match API flags. Regex_ ctor moves them to a field that later is combined with RXMatchFlags when calling the match API.
	...
	/// <summary>
	/// Flags for <see cref="Regex_.Match"/> and other <see cref="Regex_"/> class functions.
	/// Documented in PCRE help topic <see href="https://www.pcre.org/current/doc/html/pcre2api.html">pcre2api</see>.
	/// </summary>
	/// <remarks>
	/// These flags also exist in <see cref="RXFlags"/> (Regex_ constructor flags). You can set them either when calling constructor or when calling other functions.
	/// </remarks>
	[Flags]
