...
namespace Au
{
	...
	public class AuMenu :AuMTBase, IDisposable
	{
		...
		//~AuMenu() { Print("main dtor"); } //info: don't need finalizer. _cm and base have their own, and we don't have other unmanaged resources.
		...
		void _CheckDisposed()
		{
			if(IsDisposed) throw new ObjectDisposedException(nameof(AuMenu), "Disposed. Use MultiShow=true.");
		}
		...
		*/
		...
		bool _showedOnce;
		bool _inOurShow; //to detect when ContextMenuStrip.Show called not through AuMenu.Show
		...
		class ContextMenuStrip_ :ContextMenuStrip, _IAuToolStrip
		{
			...
			internal ContextMenuStrip_(AuMenu am)
			{
				_am = am;
			}
		...
		internal class ToolStripDropDownMenu_ :ToolStripDropDownMenu, _IAuToolStrip
		{
			AuMenu _am;
			bool _openedOnce;
			internal Action<AuMenu> _lazySubmenuDelegate;

			internal ToolStripDropDownMenu_(AuMenu am)
			{
				_am = am;
			}
		...
		internal void _SetItemProp(bool isBar, ToolStripItem item, Action<MTClickArgs> onClick, object icon)
		{
			...
			if(icon != null) {
				try {
					...
					case IntPtr hi: item.Image = hi == default ? null : Icon.FromHandle(hi).ToBitmap(); break;
