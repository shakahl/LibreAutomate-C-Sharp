...
namespace Au
{
	...
	/// <code><![CDATA[
	/// bool stop = false;
	/// using(new WinAccHook(AccEVENT.SYSTEM_FOREGROUND, 0, x =>
	/// {
	/// 	Print(x.wnd);
	/// 	var a = x.GetAcc();
	/// 	Print(a);
	...
	/// ]]></code>
	/// </example>
	public class WinAccHook : IDisposable
	{
		IntPtr _hh; //HHOOK
		...
		/// <example>See <see cref="WinAccHook"/>.</example>
		public WinAccHook(AccEVENT eventMin, AccEVENT eventMax, Action<HookData.AccHookData> hookProc, int idProcess = 0, int idThread = 0, AccHookFlags flags = 0)
		{
		...
		/// <exception cref="AException">Failed.</exception>
		/// <example>See <see cref="WinAccHook"/>.</example>
		public WinAccHook(AccEVENT[] events, Action<HookData.AccHookData> hookProc, int idProcess = 0, int idThread = 0, AccHookFlags flags = 0)
		{
			_proc1 = _HookProc;
		...
		void _Throw1()
		{
			...
			if(_proc1 == null) throw new ObjectDisposedException(nameof(WinAccHook));
		}
		...
		{
			if(_hh != default) {
				if(!Api.UnhookWinEvent(_hh)) PrintWarning("Failed to unhook WinAccHook.");
				_hh = default;
			} else if(_ahh != null) {
				foreach(var hh in _ahh) {
					if(hh == default) continue;
					if(!Api.UnhookWinEvent(hh)) PrintWarning("Failed to unhook WinAccHook.");
				}
				_ahh = null;
			}
		}
		...
		///
		~WinAccHook() { PrintWarning("Non-disposed WinAccHook variable."); } //unhooking makes no sense
...
namespace Au.Types
{
	...
	{
		/// <summary>
		/// Hook data for the hook procedure set by <see cref="WinAccHook"/>.
		...
		public unsafe struct AccHookData
		{
			...
			public readonly WinAccHook hook;
			...
			internal AccHookData(WinAccHook hook, AccEVENT ev, Wnd wnd, AccOBJID idObject, int idChild, int idThread, int eventTime)
			{
				this.hook = hook;
				this.ev = ev;
				this.wnd = wnd;
