...
namespace Au.Util
{
	...
	/// Also has more functions to work with memory: copy, move.
	/// </summary>
	/// <remarks>
	...
	public static unsafe class AMemory
	{
		...
		/// </summary>
		/// <remarks>
		/// If some part of memory blocks overlaps, this function is much slower than <see cref="Move"/>. Else slightly faster.
		/// </remarks>
		...
		/// </summary>
		/// <remarks>
		/// If some part of memory blocks overlaps, this function is much faster than <see cref="Copy"/>. Else slightly slower.
		/// </remarks>
	...
	/// Faster than managed and <see cref="AMemory"/> when memory size is large, more than 1 MB; else slower.
	/// The memory is initialized to zero (all bytes 0).
	/// </summary>
	public static unsafe class AVirtualMemory
	{
		/// <summary>
		...
		/// <param name="size">Byte count.</param>
		/// <exception cref="OutOfMemoryException">Failed. Probably size is too big.</exception>
		public static byte* Alloc(nint size) {
			for (int i = 0; i < 5; i++) {
				if (i > 0) {
					GC.Collect();
					GC.WaitForPendingFinalizers();
