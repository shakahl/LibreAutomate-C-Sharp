...
namespace Au.Util
{
	...
	public static partial class ARecording
	{
		...
		/// The first distance is from mouse position before the first movement; at run time it will be distance from <see cref="AMouse.LastXY"/>.
		/// To create uint value from distance dx dy use <see cref="AMath.MakeLparam"/> and cast to uint.
		/// </param>
		...
		public static string MouseToString(IEnumerable<uint> recorded, bool withSleepTimes)
		{
			...
			foreach(var u in recorded) {
				...
				if(isSleep) {
					...
					int dx = AMath.LoShort((nint)u), x = dx - pdx; pdx = dx;
					int dy = AMath.HiShort((nint)u), y = dy - pdy; pdy = dy;
