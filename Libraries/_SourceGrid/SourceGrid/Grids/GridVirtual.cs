...
namespace SourceGrid
{
	...
	public abstract class GridVirtual : CustomScrollControl
	{
		...
		public virtual void AutoSizeCells(GridRange p_RangeToAutoSize)
		{
			SuspendLayout();
		...
		{
			GridRange complete = CompleteRange;
		...
		public Size RangeToSize(GridRange range)
		{
			if (range.IsEmpty())
				return Size.Empty;
		...
		/// </summary>
		public Rectangle RangeToRectangle(GridRange range)
		{
			if (range.IsEmpty())
		...
		public GridRange RangeAtArea(CellPositionType areaType)
		{
			...
			else if (areaType == CellPositionType.FixedLeft)
			{
				...
				return new GridRange(firstRow.Value, 0, lastRow.Value, actualFixed - 1);
			}
			else if (areaType == CellPositionType.FixedTop)
			{
				...
				return new GridRange(0, firstCol.Value, actualFixed - 1, lastCol.Value);
			}
			else if (areaType == CellPositionType.Scrollable)
			{
				...
				return new GridRange(firstRow.Value, firstCol.Value,
		...
		///// </summary>
		//public GridRange RangeAtRectangle(CellPositionType areaType, Rectangle clipRectangle)
		//{
		//    if (areaType == CellPositionType.FixedTopLeft)
		//    {
		//    }
		...
		/// </summary>
		/// <returns></returns>
		private IEnumerable<GridRange> GetVisibleRegion()
		{
			GridRange rng;
		...
		///// <returns></returns>
		//public bool IsRangeVisible(GridRange range, bool partial)
		//{
		...
		{
			InvalidateRange(new GridRange(position));
		}
		...
		/// </summary>
		/// <param name="range"></param>
		public void InvalidateRange(GridRange range)
		{
			if (range.IsEmpty())
				return;
			...
			{
				GridRange visibleRange = RangeAtArea(type);
				// clip our range with visible range
				// so that we wount loop through thousands of rows
				GridRange clippedRange = GridRange.Intersect(range, visibleRange);
		...
		/// <returns></returns>
		public GridRange RangeAtAreaExpanded(CellPositionType areaType)
		{
			GridRange range = RangeAtArea(areaType);
		...
		/// </summary>
		/// <returns></returns>
		public virtual GridRange RangeToCellRange(GridRange range)
		{
			return new GridRange(range.Start, range.End);
		}
		...
		/// <returns></returns>
		public virtual GridRange PositionToCellRange(Position pPosition)
		{
			...
			if (l_Cell == null)
				...
				return new GridRange(pPosition);
			//return new GridRange(pPosition);
		}
		#endregion
		...
		{
			m_OldMouseSelectionRange = GridRange.Empty;
		}

		private GridRange m_OldMouseSelectionRange = GridRange.Empty;
		private GridRange m_MouseSelectionRange = GridRange.Empty;
		/// <summary>
		...
		/// </summary>
		[Browsable(false),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public virtual GridRange MouseSelectionRange
		{
		...
		{
			GridRange l_MouseRange = MouseSelectionRange;
		...
		{
			if (m_MouseSelectionRange != GridRange.Empty)
				OnMouseSelectionFinish(new RangeEventArgs(m_OldMouseSelectionRange));

			m_MouseSelectionRange = GridRange.Empty;
		}
		...
		{
			GridRange newMouseSelection = new GridRange(Selection.ActivePosition, p_Corner);
		...
		public virtual void ProcessSpecialGridKey(KeyEventArgs e)
		{
			...
			if (shiftPressed)
			{
				...
				Selection.SelectRange(new GridRange(m_firstCellShiftSelected, Selection.ActivePosition), true);
			}
		...
		public void PerformPaste(RangeRegion selRegion)
		{
			...
			if (rngData == null)
				return;
			GridRange rng = selRegion[0];
			GridRange data = rngData.SourceRange;
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			{
				foreach (GridRange rng in GetVisibleRegion())
				{
		...
		public virtual void ArrangeLinkedControls()
		{
			...
			foreach (LinkedControlValue linked in m_LinkedControls)
			{
				...
				{
					GridRange scrollableRange = RangeAtArea(CellPositionType.Scrollable );
		...
		{
			GridRange l_Range = p_RangeToSort.GetRange(this);
		...
		public void SortRangeRows(GridRange p_Range,
		                          int keyColumn,
		                          bool p_bAscending,
		                          IComparer p_CellComparer)
		{
		...
		/// </summary>
		[Browsable(false),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public GridRange CompleteRange
		{
			get
			{
				if (Rows.Count > 0 && Columns.Count > 0)
					return new GridRange(0, 0, Rows.Count - 1, Columns.Count - 1);
				else
					return GridRange.Empty;
			}
		}
...
//		http://www.microsoft.com/middleeast/msdn/mirror.aspx
		...
		public override void CustomScrollPageDown()
		{
			...
			GridRange rngFocusedCell = PositionToCellRange(Selection.ActivePosition);
