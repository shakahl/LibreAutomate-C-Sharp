...
namespace SourceGrid.Selection
{
	public class RowSelection : SelectionBase
	{
		...
		{
			GridRange rowRange = Grid.Rows.GetRange(row);
			if (select && mList.IsSelectedRow(row) == false)
			{
				...
				OnSelectionChanged(new RangeRegionChangedEventArgs(rowRange, GridRange.Empty));
			} else
				if (!select && mList.IsSelectedRow(row))
			{
				mList.RemoveRange(rowRange);
				OnSelectionChanged(new RangeRegionChangedEventArgs(GridRange.Empty, rowRange));
			}
		}
		...
		private GridRange NormalizeRange(GridRange range)
		{
			return new GridRange(range.Start.Row, 0, range.End.Row, Grid.Columns.Count - 1);
		}
		
		public override void SelectRange(GridRange range, bool select)
		{
			GridRange normalizedRange = NormalizeRange(range);
			if (select)
				mList.AddRange(normalizedRange); else
				mList.RemoveRange(normalizedRange);
			OnSelectionChanged(new RangeRegionChangedEventArgs(normalizedRange, GridRange.Empty));
		}
		...
		public override RangeRegion GetSelectionRegion()
		{
			...
			if (Grid.Columns.Count == 0)
				return region;
			foreach (GridRange selectedRange in mList.GetSelectedRowRegions(0, Grid.Columns.Count))
			{
