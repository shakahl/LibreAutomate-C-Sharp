    ...
    public class ColumnSelection : SelectionBase
    {
        ...
        public override void SelectColumn(int column, bool select)
        {
            if (select && mList.Contains(column) == false)
            {
                ...
                OnSelectionChanged(new RangeRegionChangedEventArgs(Grid.Columns.GetRange(column) , GridRange.Empty));
            }
            else if (!select && mList.Contains(column))
            {
                mList.Remove(column);

                OnSelectionChanged(new RangeRegionChangedEventArgs(GridRange.Empty, Grid.Columns.GetRange(column)));
            } 
        }
        ...
        public override RangeRegion GetSelectionRegion()
        {
            ...
            if (Grid.Rows.Count > 0)
            {
                ...
                {
                    region.Add(ValidateRange(new GridRange(Grid.FixedRows, col, Grid.Rows.Count - 1, col)));
                }
            }
