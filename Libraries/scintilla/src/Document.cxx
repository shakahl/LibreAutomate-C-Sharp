...
Document::Document(int options) :
	...
	au_userData = 0; //Au: user data for QM2
...
#ifndef _M_AMD64 //Au: used by QM2
	//QM: set folding levels for all lines using callback.
	//With large document almost 100 times faster.
	//wParam (line) is callback parameter, lParam (level) is callback function.
	if(level >= 0x10000) {
		int (*fCallback)(int param, int offset); *(int*)&fCallback = level;
		LineLevels* p = Levels();
		int nLines = LinesTotal();
		for(int i = 0; i < nLines; i++) {
			int r = fCallback(line, LineStart(i));
			if(r) //if callback returns 0, don't change. To change, it returns >=1024.
				if(r != p->SetLevel(i, r, nLines))
					level = 0; //redraw margin. If all line levels are the same, return unchanged to not redraw.
		}
...
bool SCI_METHOD Document::Sci_SetFoldLevels(int line, int lineTo, int len, int* a) {
	...
	for(int i = 0; line < lineTo; line++) {
		...
		if(lev != prevLevel) {
			...
			if(prevLevel <= SC_FOLDLEVELBASE)levels->SetLevel(line, lev, nLines); //first folding. Slightly faster, but does not show hidden lines when header unset, etc.
