	...
	public class TreeColumn : Component
	{
		...
		internal Bitmap CreateGhostImage(Rectangle bounds, Font font)
		{
			...
			using(var gr = Graphics.FromImage(b)) {
				DrawBackground(gr, bounds, true, true);
				DrawContent(gr, bounds, font, true);
		...
		internal void Draw(Graphics gr, Rectangle bounds, Font font, bool pressed, bool hot)
		{
			...
			DrawContent(gr, bounds, font, false);
		}

		private void DrawContent(Graphics gr, Rectangle bounds, Font font, bool memoryBitmap)
		{
		...
		internal static void DrawBackground(Graphics gr, Rectangle bounds, bool pressed, bool hot)
		{
			if(Application.RenderWithVisualStyles) {
				if(pressed || hot) {
					var renderer = new VisualStyleRenderer(pressed ? VisualStyleElement.Header.Item.Pressed : VisualStyleElement.Header.Item.Hot);
					...
					using(var pen = new Pen(Color.FromArgb(c.R * 5 / 6, c.G * 5 / 6, c.B * 5 / 6))) {
						int x = bounds.Right - 1, y = bounds.Bottom - 1;
						gr.DrawLine(pen, x, bounds.Y, x, bounds.Bottom);
						gr.DrawLine(pen, bounds.X, y, bounds.Right, y);
					}
					//renderer.DrawEdge(gr, bounds, Edges.Bottom, EdgeStyle.Etched, EdgeEffects.Soft);
				}
			} else {
				gr.FillRectangle(SystemBrushes.Control, bounds);
