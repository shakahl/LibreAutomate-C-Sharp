...
namespace Aga.Controls.Tree
{
	public partial class TreeViewAdv
	{
		...
		protected override void OnKeyPress(KeyPressEventArgs e)
		{
			base.OnKeyPress(e);
			//if(!e.Handled) {
			//	if(_search == null) _search = new IncrementalSearch(this);
		...
		{
			if(_dragEscaped) return;
			TreeNodeAdvMouseEventArgs args = CreateMouseArgs(e);
			if(args.Node != null) {
				if(!(args.Control is NodePlusMinus)) OnNodeMouseClick(args);
			}
			base.OnMouseClick(e);
		}
		...
		protected override void OnMouseDoubleClick(MouseEventArgs e)
		{
			...
			if(args.Node != null) {
				...
				if(!args.Handled) {
					if(!(args.Control is NodePlusMinus)) OnNodeMouseDoubleClick(args);

					if(!args.Handled && args.Button == MouseButtons.Left && ShowPlusMinus) args.Node.IsExpanded = !args.Node.IsExpanded;
				}
			} else Input.MouseDoubleClick(args);
		...
		private void SetDropPosition(Point pt)
		{
			...
			if(node != null) {
				...
				if(pos <= DragDropTopEdgeSensivity)
					_dropPosition.Position = NodePosition.Before;
				else if(pos >= (1 - DragDropBottomEdgeSensivity))
		...
		private void DragAutoScroll()
		{
			...
			var k = GetScrollInfo(true);
			int i = -1;
			if(pt.Y < 20 && k.Pos > 0) i = k.Pos - 1;
			else if(pt.Y > Height - 20 && k.Pos <= k.Max - k.Page) i = k.Pos + 1;
			if(i >= 0) SetScrollPos(true, i, true);
		}
		...
		protected override void OnQueryContinueDrag(QueryContinueDragEventArgs e)
		{
			...
			if(e.EscapePressed && e.Action == DragAction.Cancel) _dragEscaped = true;
		}
