...
const STR c_agentWindowClassName = L"AuInProc";
...
inline LARGE_INTEGER LI(__int64 i) {
...
namespace inproc
{
	...
	class _AccHooks
	{
		...
		//TODO: use map
		CAtlList<_RESTORE> _a; //usually there are 1 or 2 interface tables in a process

		bool _Hook(LPVOID* fp, LPVOID f) {
			DWORD oldProt = 0;
			if(!VirtualProtect(fp, sizeof(LPVOID), PAGE_EXECUTE_READWRITE, &oldProt)) return false;
			*fp = f;
			if(oldProt != PAGE_EXECUTE_READWRITE) VirtualProtect(fp, sizeof(LPVOID), oldProt, &oldProt);
			return true;
		}

	public:
		bool Hook(IAccessible* iacc) {
			LPVOID* fp = *(LPVOID**)iacc + 16; //&get_accHelpTopic
			LPVOID f = *fp; //get_accHelpTopic
			if(f != Hook_get_accHelpTopic) {
				if(!_Hook(fp, Hook_get_accHelpTopic)) return false;
				_RESTORE r = { fp, f };
				_a.AddTail(r);
			}
			return true;
		...
		~_AccHooks() {
			for(auto pos = _a.GetHeadPosition(); pos != null;) {
				_RESTORE r = _a.GetNext(ref pos);
				bool restored = _Hook(r.fp, r.fp);
				//Printf(L"restored=%i", restored);
			}
		}
	};
	...
	void _AccFind1(eAgentAction action, int connectionId)
	{
		...
		auto pSM = (_AccFindParams*)(h + 1);
		int size = pSM->size;
		CHeapPtr<_AccFindParams> p; p.AllocateBytes(size); memcpy(p, pSM, size);
	...
	LRESULT WINAPI AgentWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		if(msg == WM_NCDESTROY) {
			...
			if(0 == InterlockedDecrement(&s_nAgentThreads)) {
				//unload dll
				CloseHandle(CreateThread(null, 64 * 1024, UnloadThreadProc, null, 0, null));
			}
		}
...
void WINAPI Cpp_RunDllW(HWND hwnd, HINSTANCE hinst, STR lpszCmdLine, int nCmdShow) //note: with W
{
	...
	if(!_InjectDll(w, out wa, tid, pid)) return;
}
...
HRESULT GetAgentWindow(HWND w, out HWND& wa)
{
	...
	static thread_local TidHwndCache t_wndCache, t_wndCacheFailed;
	...
	if(!wa) {
		...
		if(differentBits) {
			...
			if(_RunDll(w)) { //speed: 60-100 ms
				wa = FindWindowEx(HWND_MESSAGE, 0, c_agentWindowClassName, name);
				ok = !!wa;
			}
		} else {
			ok = _InjectDll(w, out wa, tid, pid);
		}
		//Perf.Next();
...
HRESULT Cpp_AccFind(bool inject, HWND w, IAccessible* iaccParent, STR role, STR name, int flags, AccCallbackT also, int skip, out Cpp_Acc& aResult)
{
	...
	if(inject) {
		...
		int memSize = _AccFindParams::CalcMemSize(roleLen, nameLen);

		if(memSize + sizeof(SM_HEADER) > c_sharedMemorySize) return ER_ParamsTooBig;
		auto p = (_AccFindParams*)(hSM + 1);
		p->size = memSize;
		p->hwnd = (int)(LPARAM)w;
