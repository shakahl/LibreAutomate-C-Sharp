...
namespace Au
{
	...
	/// <br/>Web page AOs initially are disabled (missing). Workarounds:
	...
	/// <br/>Your process must have the same 32/64 bitness as the installed Java. To remove this limitation, install Java 32-bit and 64-bit (they coexist).
	/// - OpenOffice.
	/// <br/>Often crashes after using AOs, usually when closing. Noticed in OpenOffice 4.1.4; may be fixed in newer versions.
	/// - LibreOffice.
	/// <br/>AOs are unavailable unless this process is 32-bit (when LibreOffice is 64-bit). Also need flag <see cref="AFFlags.NotInProc"/>.
	...
	/// <br/>Workaround: search only in that control. For example, use prop "class" or id". If it's a web browser control, use role prefix "web:". Or find the control with <see cref="Wnd.Child"/> and search in it. Or use <see cref="Acc.Finder.Find(Wnd, Wnd.ChildFinder)"/>.
	...
	/// <br/>Known bugs: 1. Toolbar buttons don't have Name in some cases. 2. <see cref="Focus"/> and <see cref="Select"/> often don't work properly.
	/// <br/>Workaround: don't use <see cref="AFFlags.NotInProc"/>, or use <see cref="AFFlags.UIA"/>.
