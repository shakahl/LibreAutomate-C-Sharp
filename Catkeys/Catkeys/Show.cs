...
namespace Catkeys
{
	...
	/// <summary>
	/// Edit control type for Show.InputDialogX and Show.TaskDialogObject.SetEditControl().
	/// </summary>
	public enum TDEdit
	{
	...
	{
		/// <summary>
		/// Constants for Show.TaskDialogObject messages and events.
		/// </summary>
		public class TDApi
		{
			/// <summary>
			...
			/// <summary>
			/// Constants for Show.TDApi.TDM.SET_ELEMENT_TEXT and Show.TDApi.TDM.UPDATE_ELEMENT_TEXT messages and Show.TaskDialogObject.Send.Text().
			/// </summary>
			public enum TDE
			{
				CONTENT,
			...
			/// <summary>
			/// Constants for Show.TDApi.TDM.UPDATE_ICON message.
			/// </summary>
			public enum TDIE
			{
				ICON_MAIN,
		...
		/// <remarks>
		/// Uses Show.TaskDialogObject class, which uses Windows API function TaskDialogIndirect (you can find more info in MSDN).
		...
		/// <param name="customButtons">Adds buttons that have custom text and id. A list of strings "id text" separated by |, like "1 One|2 Two|3 Three". Also can be string[] or List˂string˃. Use TDFlag.CommandLinks in flags to change button style. You can also use common button ids, which are negative, for example id -2 (TDResult.Cancel) makes the button work like the common Cancel button (adds title bar x button, enables Esc key).</param>
		/// <remarks>
		/// Calls TaskDialogEx(), which uses Show.TaskDialogObject class, which uses Windows API function TaskDialogIndirect (you can find more info in MSDN).
		/// The returned TDResult object has these properties: clicked button id (eg TDResult.OK, 1 (custom button), TDResult.Timeout), name, selected radio button id, check box state.
		...
		/// Note: the program must have manifest that tells to use comctl32.dll version 6 or later. C# projects by default use old dll, and this function fails.
		/// </remarks>
		public static TDResult TaskDialog(string text1, object text2 = null, TDStyle style = null, Wnd owner = default(Wnd), StringList customButtons = null)
		{
		...
		/// <example>
		/// <code>
		/// var d = new Show.TaskDialogObject(); //this example uses the 0-parameters constructor, but also there are constructors that have all the same parameters as Show.TaskDialogEx()
		...
		/// <summary>
		/// Arguments for event handlers of Show.TaskDialogObject, Show.TaskDialogEx() (onLinkClick) and some other functions.
		...
		public partial class TaskDialogObject
		{
			...
			/// Example: <c>var d = new Show.TaskDialogObject("text"); Task.Run(() => { d.Show(); }); if(!d.ThreadWaitOpen()) return;</c>
			/// </summary>
			...
			public struct Sender
			{
				...
				/// Example (in an event handler): <c>ed.obj.Send.Message(Show.TDApi.TDM.CLICK_VERIFICATION, 1);</c>
				...
				/// <summary>
				/// Sends message Show.TDApi.TDM.SET_ELEMENT_TEXT or Show.TDApi.TDM.UPDATE_ELEMENT_TEXT to the dialog.
				/// Call this method while the dialog is open, eg in an event handler.
				/// Example (in an event handler): <c>ed.obj.Send.Text(true, Show.TDApi.TDE.CONTENT, "New text.");</c>
				/// </summary>
				...
				/// Sends message Show.TDApi.TDM.NAVIGATE_PAGE.
				/// </summary>
				...
				/// Call this method while the dialog is open, eg in an event handler.
				/// Sends message Show.TDApi.TDM.CLICK_BUTTON.
				/// </summary>
				...
				/// Sends message Show.TDApi.TDM.ENABLE_BUTTON.
				/// </summary>
				...
				/// Call this method while the dialog is open, eg in an event handler.
				/// Sends message Show.TDApi.TDM.SET_PROGRESS_BAR_POS.
				/// </summary>
		...
		/// <remarks>
		/// Uses Show.TaskDialogObject class, like Show.TaskDialogEx(). More info there.
		/// </remarks>
		...
		/// <param name="editType">Edit field type. It can be simple text (TDEdit.Text, default), multiline, number, password or combo box.</param>
		/// <remarks>
		/// Calls Show.InputDialogEx(), which uses Show.TaskDialogObject class, like Show.TaskDialogEx(). More info there.
		/// </remarks>
	...
	public static partial class Show
	{
		...
		/// <param name="owner">Owner window or Wnd0.</param>
		/// <remarks>
		/// Calls Show.ListDialogEx(), which uses Show.TaskDialogObject class, like Show.TaskDialogEx(). More info there.
		/// </remarks>
		public static int ListDialog(StringList list, string text1 = null, object text2 = null, TDStyle style = null, Wnd owner = default(Wnd))
		{
	...
	public static partial class Show
	{
		...
		/// Returns Show.TaskDialogObject object that can be used to control the dialog: set progress bar position, update text, close etc.
		...
		/// Shows dialog with progress bar.
		/// Creates dialog in other thread and returns without waiting until it is closed.
		/// Returns Show.TaskDialogObject object that can be used to control the dialog: set progress bar position, close etc.
		...
		/// </example>
		/// <remarks>
		/// Calls Show.ProgressDialogEx(), which uses Show.TaskDialogObject class, like Show.TaskDialogEx(). More info there.
		/// </remarks>
	...
	public static partial class Show
	{
		...
		/// Creates dialog in other thread and returns without waiting until it is closed.
		/// Returns Show.TaskDialogObject object that can be used to control the dialog, eg close.
		...
		/// </code></example>
		/// <remarks>
		/// Calls Show.TaskDialogNoWaitEx(), which uses Show.TaskDialogObject class, like Show.TaskDialogEx(). More info there.
		/// </remarks>
