...
namespace Catkeys
{
	...
	public class CatException :Exception
	{
		...
		/// Sets NativeErrorCode = (winApiErrorCode != 0) ? winApiErrorCode : Native.GetError().
		/// </summary>
		public CatException(int winApiErrorCode, string message) : base(message ?? "Failed.") { NativeErrorCode = (winApiErrorCode != 0) ? winApiErrorCode : Native.GetError(); }

		/// <summary>
		/// Sets Message = message + " " + innerException.Message.
		/// Sets NativeErrorCode = 0.
		/// </summary>
		...
		/// If appendMessage is null, uses Native.GetErrorMessage(NativeErrorCode) if NativeErrorCode not 0, or InnerException.Message if InnerException is not null.
		...
		protected string FormatMessage(string appendMessage = null, string commonPostfix = null)
		{
			...
			if(appendMessage == null) {
				if(NativeErrorCode != 0) appendMessage = Native.GetErrorMessage(NativeErrorCode);
				else if(InnerException != null) appendMessage = InnerException.Message;
			}
	...
	/// <summary>
	/// Exception thrown mostly by <see cref="Wnd"/> functions, when invalid window handle or fails for other reasons.
	/// </summary>
	/// <remarks>
	/// Some constructors support Windows API error code. Then Message will contain error description.
	/// If error code ERROR_INVALID_WINDOW_HANDLE, Message returns a more specific text that depends on whether the window handle is 0.
	/// If constructor parameter 'winApiErrorCode' is 0 and the window is invalid, uses ERROR_INVALID_WINDOW_HANDLE.
	...
	public class WndException :CatException
	{
		...
		/// </summary>
		public WndException(Wnd w) : base() { Window = w; NativeErrorCode = _Code(0, w); }

		/// <summary>
		/// Sets Message = message.
		/// Sets NativeErrorCode = w.IsAlive ? 0 : ERROR_INVALID_WINDOW_HANDLE.
		/// </summary>
		public WndException(Wnd w, string message) : base(message) { Window = w; NativeErrorCode = _Code(0, w); }

		/// <summary>
		/// Sets Message = "Failed. " + Native.GetErrorMessage(winApiErrorCode).
		/// Sets NativeErrorCode = (winApiErrorCode != 0) ? winApiErrorCode : (w.IsAlive ? Native.GetError() : ERROR_INVALID_WINDOW_HANDLE).
		/// </summary>
		public WndException(Wnd w, int winApiErrorCode) : base(_Code(winApiErrorCode, w)) { Window = w; }

		/// <summary>
		/// Sets Message = message + " " + Native.GetErrorMessage(winApiErrorCode).
		/// Sets NativeErrorCode = (winApiErrorCode != 0) ? winApiErrorCode : (w.IsAlive ? Native.GetError() : ERROR_INVALID_WINDOW_HANDLE).
		/// </summary>
		public WndException(Wnd w, int winApiErrorCode, string message) : base(_Code(winApiErrorCode, w), message) { Window = w; }

		/// <summary>
		/// Sets Message = message + " " + innerException.Message.
		/// Sets NativeErrorCode = 0.
		/// </summary>
		public WndException(Wnd w, string message, Exception innerException) : base(message) { Window = w; }
