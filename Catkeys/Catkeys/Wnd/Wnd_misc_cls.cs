	...
	public partial struct Wnd
	{
		public static partial class Misc
		{
			...
			public class WndClass
			{
				...
				/// Not thread-safe.
				/// </remarks>
				public static WndClass Register(string className, Native.WNDPROC wndProc, int wndExtra = 0, uint style = 0, WndClassEx ex = null)
				{
				...
				void _Register(ref Api.WNDCLASSEX x, string className, Native.WNDPROC wndProc, int wndExtra, uint style)
				{
				...
				/// Not thread-safe.
				/// </remarks>
				public static WndClass Superclass(string baseClassName, string className, Native.WNDPROC wndProc, int wndExtra = 0, bool globalClass = false, IntPtr baseModuleHandle = default(IntPtr))
				{
					var r = new WndClass();
				...
				/// </summary>
				/// <param name="className">Class name.</param>
				/// <param name="wndProc">Window procedure. Can be classic window procedure that calls Api.DefWindowProc(). Don't need to protect it from GC. If null, will use Api.DefWindowProc.</param>
				/// <param name="wndExtra">Size of extra window memory which can be accessed with SetWindowLong/GetWindowLong with >=0 offset. Example: IntPtr.Size.</param>
				...
				/// <para />Thread-safe.
				/// </remarks>
				public static ushort InterDomainRegister(string className, Native.WNDPROC wndProc, int wndExtra = 0, uint style = 0, WndClassEx ex = null)
				{
				...
				{
					Native.WNDPROC wndProc; if(!_interDomain.TryGetValue(className, out wndProc)) return Wnd0;
					Wnd w = Api.CreateWindowEx(exStyle, className, name, style, x, y, width, height, parent, id, Zero, 0);
					if(!w.Is0 && wndProc != null) w.SetWindowLong(Api.GWL_WNDPROC, Marshal.GetFunctionPointerForDelegate(wndProc));
					return w;
				}

				/// <summary>
				/// Rarely used Api.WNDCLASSEX fields. Used with some WndClass functions.
				/// </summary>
...
#pragma warning restore 1591 //XML doc
			...
			public class WindowsFormsControlNames :IDisposable
			{
				...
				/// <exception cref="CatException">Throws when cannot allocate process memory (see Process_.Memory) needed to get control names, usually because of UAC.</exception>
