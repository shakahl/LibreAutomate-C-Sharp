...
namespace Catkeys
{
	...
	public static partial class AElement
	{
		...
		/// <param name="find">
		/// If false, uses IUIAutomation.ElementFromIAccessible. It fails for most objects. Also, the returned element may be useless, for example searching its descendants can be 60 times slower than usually.
		/// If true, finds element that matches ao rectangle and other properties. It is slow, but usually succeeds.
		...
		public static UIA.IElement FromAcc(Acc ao, bool? find = null)
		{
			...
			if(!find.HasValue || find.GetValueOrDefault()) { //null or true
				try {
					var state = ao.State;
					if(state.HasAny_(AccSTATE.INVISIBLE | AccSTATE.OFFSCREEN)) return null; //then IElement.BoundingRectangle is empty
					...
					var cond = new ACondition().Rect(r).LARole(ao.RoleEnum).Add(UIA.PropertyId.LegacyIAccessibleState, state);
		...
		public static UIA.IElement FromPoint(Point pScreen, bool preferLINK = false)
		{
			...
			if(wTL.ClassNameIs("Chrome*")) {
				...
				if(isD3D) {
					...
					using(var acc = Acc.FromXY(pScreen.X, pScreen.Y, noThrow: true)) { //5 ms
						if(acc != null) {
