...
namespace Catkeys
{
	public unsafe partial class Acc
	{
		...
		public AccROLE RoleInt
		{
			...
			{
				LibThrowIfDisposed();
				if(_misc.role != 0) return (AccROLE)_misc.role;
		...
		public bool IsInvisible
		{
			...
			{
				var k = State;
				if(!k.Has_(AccSTATE.INVISIBLE)) return false;
				if(!k.Has_(AccSTATE.OFFSCREEN)) return true;
				switch(RoleInt) {
				case AccROLE.WINDOW:
		...
		internal static string LibNameOfWindow(Wnd w)
		{
			...
			var hr = Cpp.Cpp_AccFromWindow(3, w, 0, out _, out var b);
			return _BstrToString(hr, b);

			//speed: inproc ~10% faster. But first time slower, especially if process of different bitness.
			//info: Cpp_AccFromWindow with flags 3, if fails to inject, retries outproc.
		}
		...
		public string Value
		{
			...
			{
				LibThrowIfDisposed();
				CatException.ThrowIfHresultNot0(Cpp.Cpp_AccAction(this, 'v', value));
			}
		}
		...
		public void DoAction()
		{
			...
			if(hr != 0) CatException.ThrowIfHresultNot0(hr);
		...
		/// </summary>
		/// <exception cref="CatException">Failed to get rectangle, or the object is invisible/offscreen.</exception>
		/// <remarks>
		...
		{
			var w = WndContainer;
			if(!GetRect(out var r, w)) throw new CatException(0, "*get rectangle");
			if(r.IsEmpty || State.HasAny_(AccSTATE.INVISIBLE | AccSTATE.OFFSCREEN)) throw new CatException(0, "Invisible or offscreen");
		...
		/// <exception cref="CatException">Failed. For example, when this object is invalid, or its top-level window does not contain a web page.</exception>
		...
		{
			Wnd w = WndTopLevel; if(w.Is0) throw new CatException("*get window");
			Acc doc = Acc.Wait(-1, w, "web:"); if(doc == null) throw new CatException("*find web page");
			...
			var to = new WaitFor.LibTimeout(secondsTimeout, 25);
			while(to.Sleep()) {
				w.ThrowIfInvalid();
				if(!wndOK) {
					var s = w.Name;
		...
		public bool DoActionAndWaitForWebPageLoaded(double secondsTimeout = 60, Action<Acc> action = null, Wnd w = default)
		{
			...
			if(w.Is0) {
				w = WndContainer;
				if(w.Is0) throw new CatException("*get window");
			}
			...
			var hh = Api.SetWinEventHook(hookEvent, hookEvent, default, hook, 0, tid, 0); if(hh == default) throw new CatException();
			try {
		...
		/// <remarks>
		/// Returns an array of props length. Element values are property values, or ""/empty (not null) if failed to get. Element types are as of the corresponding Acc properties.
		/// 
		...
		/// var a = Acc.Find(w, "LINK").OrThrow();
		/// foreach(var v in a.GetMultipleProperties("rnvdhaksRwoi@")) { //all properties
		/// 	if(v is Dictionary<string, string> d) Print(d); //HTML attributes
		/// 	else Print(v);
		/// }
		/// ]]></code>
		/// </example>
		public object[] GetMultipleProperties(string props)
		{
			LibThrowIfDisposed();
			var r = new object[props.Length];
			if(_elem != 0 || !_misc.flags.Has_(_MiscFlags.InProc)) props = props.Replace("o", "").Replace("i", "").Replace("@", "");
			if(props.Length != 0) {
				...
				using(b) {
					...
					for(int i = 0; i < props.Length; i++) {
						...
						object t = null;
						switch(props[i]) {
						case 'r': t = len > 0 ? *(RECT*)p : default; break;
						case 's': t = len > 0 ? *(AccSTATE*)p : default; break;
						case 'w': t = len > 0 ? (Wnd)(LPARAM)(*(int*)p) : default; break;
						default:
							if(props[i] == '@') t = _AttributesToDictionary(p, len);
							else if(len == 0) t = "";
							else t = new string(p, 0, len);
							break;
						}
						Debug.Assert(t != null);
						//Print(t);
		...
		/// <item>"parent" or "pa" - parent (container) AO.</item>
		...
		/// <example>
		/// <code><![CDATA[
		/// a = a.Navigate("parent next ch3", true).OrThrow();
		/// ]]></code>
		/// </example>
		public Acc Navigate(string navig, double secondsToWait = 0, bool disposeThis = false)
		{
			...
			if(secondsToWait == 0) {
				...
				var to = new WaitFor.LibTimeout(secondsToWait > 0 ? -secondsToWait : 0.0);
		...
		/// </summary>
		/// <exception cref="CatException">Failed to scroll, or the object does not support scrolling.</exception>
		/// <remarks>
		...
		public void ScrollTo()
		{
			...
			int hr;
			if(_misc.flags.Has_(_MiscFlags.UIA)) hr = Cpp.Cpp_AccAction(this, 's');
