...
namespace Catkeys
{
	...
	{
		[ThreadStatic] static Lazy<UIA.CUIAutomation> s_uia = new Lazy<UIA.CUIAutomation>(() => new UIA.CUIAutomation(), true);
		...
		/// </remarks>
		public UIA.IUIAutomationElement ToUIElement()
		{
			var a = Unsafe.As<UIA.IAccessible>(Marshal.GetUniqueObjectForIUnknown(_iacc));
			Debug.Assert(a != null);
			try {
				return s_uia.Value.ElementFromIAccessible(a, _elem);
			}
			catch { return null; } //eg native ElementFromIAccessible returned E_INVALIDARG and it was translated to ArgumentException
