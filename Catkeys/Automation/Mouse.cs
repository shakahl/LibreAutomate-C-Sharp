...
namespace Catkeys
{
	...
	public static partial class Mouse
	{
		...
		/// </exception>
		/// <exception cref="CatException">Failed to move the cursor to x y. Some reasons: 1. Another thread blocks or modifies mouse input (API BlockInput, mouse hooks, frequent API SendInput etc); 2. The active window belongs to a process of higher UAC integrity level; 3. Some application called API ClipCursor. No exception if Options.Relaxed is true (then final cursor position is undefined).</exception>
		/// <remarks>
		/// Uses <see cref="ScriptOptions.MouseMoveSpeed">Options.MouseMoveSpeed</see>, <see cref="ScriptOptions.MouseClickSleep">Options.MouseClickSleep</see> (sleeps Math.Min(Options.MouseClickSleep, 7)), <see cref="ScriptOptions.Relaxed">Options.Relaxed</see>.
		/// </remarks>
		...
		/// <exception cref="CatException">Failed to move to the calculated x y. Some reasons: 1. Another thread blocks or modifies mouse input (API BlockInput, mouse hooks, frequent API SendInput etc); 2. The active window belongs to a process of higher UAC integrity level; 3. Some application called API ClipCursor. No exception if Options.Relaxed is true (then final cursor position is undefined).</exception>
		/// <remarks>
		/// Uses <see cref="ScriptOptions.MouseMoveSpeed">Options.MouseMoveSpeed</see>, <see cref="ScriptOptions.MouseClickSleep">Options.MouseClickSleep</see> (sleeps Math.Min(Options.MouseClickSleep, 7)), <see cref="ScriptOptions.Relaxed">Options.Relaxed</see>.
		/// </remarks>
		...
		/// <exception cref="CatException">Failed to move to the last x y. Some reasons: 1. Another thread blocks or modifies mouse input (API BlockInput, mouse hooks, frequent API SendInput etc); 2. The active window belongs to a process of higher UAC integrity level; 3. Some application called API ClipCursor. No exception if Options.Relaxed is true (then final cursor position is undefined).</exception>
		/// <remarks>
		/// Uses <see cref="ScriptOptions.MouseClickSleep">Options.MouseClickSleep</see> (sleeps Math.Min(Options.MouseClickSleep, 7) after each movement, except if recorded with sleep times), <see cref="ScriptOptions.Relaxed">Options.Relaxed</see> (for the last movement; does not care about intermediate movements). Does not use <see cref="ScriptOptions.MouseMoveSpeed">Options.MouseMoveSpeed</see>.
		/// </remarks>
	...
	public static partial class Mouse
	{
		...
		public static class Message
		{
			...
			static void _Send(Wnd w, uint message, uint wParam, uint lParam, bool isCtrl, bool isShift)
			{
				...
				if(!w.Post(message, wParam, lParam)) throw new CatException(0);
			...
			///// If 0 (default), calls API <msdn>PostMessage</msdn> (it does not wait) and waits <see cref="ScriptOptions.MouseClickSleep">Options.MouseClickSleep</see>.
			...
			///// Else calls API <msdn>SendMessageTimeout</msdn> which waits max waitMS milliseconds, then throws CatException.
			...
			///// </param>
			///// <exception cref="CatException">Failed, or timeout.</exception>
			...
			//	}
			//	if(!ok) throw new CatException(0);
			//}
		}
	}
#endif
