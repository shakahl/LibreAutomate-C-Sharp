...
/// preScript file //run this script before compiling. Its args[0] will be the full path of this C# file; args[1] - source code.
/// postScript file //run this script after compiling successfully. Its args[0] will be the full path of this C# file; args[1] - source code.
/// ]]></code>
/// When compiling multiple files (project, 'c'), these options can be only in the main file; other files inherit them.
/// 
...
/// run file //run this script instead. For example, allows to test dll easily. Its args[0] will be the full path of the output assembly.
/// ]]></code>
/// When compiling multiple files (project, 'c'), these options can be only in the main file.
...
/// sign file.pfx //sign the output .exe/.dll file using this file. Can be filename or relative path, like with 'c'.
...
///	If used option 'icon' or 'manifest', also adds native version resource, with values collected from attributes such as [assembly: AssemblyVersion("...")] etc.
/// 
...
/// Files used with other options ('c', 'resource' etc) must be in this collection; links to external files are supported.
/// </example>
class MetaComments
{
	/// <summary>
	...
	/// </summary>
	public List<MCFile> Files { get; private set; }
	...
	/// null if none.
	/// </summary>
	public List<FileNode> ResourceFiles { get; private set; }

	/// <summary>
	/// Meta option 'preScript'.
	/// </summary>
	public FileNode PreScript { get; private set; }

	/// <summary>
	/// Meta option 'postScript'.
	/// </summary>
	public FileNode PostScript { get; private set; }

	/// <summary>
	/// Meta option 'run'.
	/// </summary>
	public FileNode Run { get; private set; }
	...
	/// Meta option 'sign'.
	/// </summary>
	public FileNode SignPfxFile { get; private set; }
	...
	public bool Parse(FileNode f, FileNode projFolder)
	{
		...
		if(projFolder != null) {
			foreach(var ff in projFolder.EnumProjectFiles(f)) _ParseFile(ff, false);
		}
		...
		if(!_FinalCheckOptions()) return false;
	...
	public void _ParseFile(FileNode f, bool isMain)
	{
	...
	void _ParseOption(string code, in StringSegment seg)
	{
		...
		case "r":
			try {
				...
				//FUTURE: if value ends with ".cs", and the ".cs" file is the main file of a dll project, build the dll if need.
			}
			catch(Exception e) {
		...
		case "c":
			...
			return;
		case "resource":
			var rFile = _GetFile(value, iValue); if(rFile == null) return;
			if(ResourceFiles == null) ResourceFiles = new List<FileNode>();
			else if(ResourceFiles.Contains(rFile)) return;
			ResourceFiles.Add(rFile);
			return;
		...
		case "define":
			...
			break;
		case "preScript":
			PreScript = _GetFile(value, iValue);
			break;
		case "postScript":
			PostScript = _GetFile(value, iValue);
			break;
		case "outputPath":
#if STANDARD_SCRIPT
		...
		case "resFile":
			...
			break;
		case "sign":
			SignPfxFile = _GetFile(value, iValue);
			break;
		case "xmlDoc":
		...
		//	break;
		//case "version": //will be auto-created if using 'icon' or 'manifest' in meta and [assembly: AssemblyVersion] etc. When adding icon/manifest, we call CreateDefaultWin32Resources(true).
		//	break;
		//case "include": //FUTURE
	...
	FileNode _GetFile(string s, int errPos)
	{
	...
	string _GetOutPath(string s, int errPos)
	{
		...
		if(!Path_.IsFullPathExpandEnvVar(ref s)) {
			if(s.StartsWith_('\\')) s = _fn.Model.FilesDirectory + s;
...
struct MCFile
{
	...
	public MCFile(FileNode f, string code) { this.f = f; this.code = code; }
}

enum EOutputType { app, console, dll }

enum EIsolation { appDomain, process, thread, hostThread }

enum EUac { host, user, admin, uiAccess, low }

enum ERunAlone { yes, no, wait }
