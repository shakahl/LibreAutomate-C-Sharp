...
static partial class Compiler
{
	...
	/// </summary>
	internal unsafe class XCompiled
	{
		FilesModel _model;
		string _file;
		Dictionary<string, string> _data;

		public XCompiled(FilesModel model)
		{
			_model = model;
			_file = _model.TempDirectory + @"\compiled.log";
			CacheDirectory = _model.TempDirectory + @"\cache";
		}
		...
		public bool IsCompiled(FileNode f, out CompResults r, FileNode projFolder)
		{
			...
			bool isScript = f.NodeType == ENodeType.Script;
			...
			} else asmFile = CacheDirectory + "\\" + f.GuidHex;
			//Print(asmFile);
				...
				foreach(var s in value.Segments_(iPipe, value.Length - iPipe, "|", SegFlags.NoEmpty)) {
					...
					case 'p':
						...
						if(projFolder != null) {
							...
							foreach(var f1 in projFolder.EnumProjectFiles(f)) {
					...
					default:
						...
						var f2 = _model.FindByGUID(value.Substring(offs, s.EndOffset - offs));
...
* - r
			*/
			...
			using(new Au.Util.LibStringBuilder(out var b)) {
				if(m.OutputPath != null) b.Append("|=").Append(outFile); //else f.HexGUID in cache
				...
				if(m.ResourceFiles != null) foreach(var v in m.ResourceFiles) _AppendFile("|d", v); //guids of 'resource' files
				_AppendFile("|k", m.IconFile);
				_AppendFile("|m", m.ManifestFile);
				_AppendFile("|x", m.ResFile);
				_AppendFile("|s", m.SignPfxFile);
		...
		bool _Open()
		{
			...
			foreach(var s in sData.Segments_("\n\r", SegFlags.NoEmpty)) {
				if(_data == null) {
					...
					_data = new Dictionary<string, string>(sData.CountLines_());
					continue;
				}
				if(s.Length < 22) goto g1; //Base64 GUID length
				string s1, s2 = null;
				if(s.Length == 22) s1 = s.Value; else { s1 = s.Substring(0, 22); s2 = s.Substring(22); }
				_data[s1] = s2;
			}
		...
		void _Save()
		{
			File_.CreateDirectory(_model.TempDirectory);
