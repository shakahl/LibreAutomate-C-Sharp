...
namespace Catkeys
{
	...
	public static class MessageDialog
	{
		...
		/// </remarks>
		public static MDResult Show(string text, MDButtons buttons, MDIcon icon = 0, MDFlag flags = 0, Wnd owner = default(Wnd), string title = null)
		{
		...
		/// These script options are applied: Script.Option.dialogRtlLayout, Script.Option.dialogTopmostIfNoOwner, ScriptOptions.DisplayName (title).
		/// </remarks>
		public static char Show(string text, string style = null, Wnd owner = default(Wnd), string title = null)
		{
			MDButtons buttons = 0;
			MDIcon icon = 0;
			MDFlag flags = 0;
	...
	/// <code>
	/// var d = new TaskDialog(); //this example uses the 0-parameters constructor, but also there are constructors that have all the same parameters as TaskDialog.ShowEx()
	...
	public partial class TaskDialog
	{
		...
		/// <summary>
		/// Initializes new object and sets commonly used properties.
		/// All parameters are the same as of <see cref="ShowEx()"/>.
		/// </summary>
		public TaskDialog(
			Wnd owner, string text1, object text2 = null, TDButton buttons = 0, TDIcon icon = 0, TDFlag flags = 0,
		...
		/// </summary>
		public TaskDialog(
			string text1, object text2 = null, TDStyle style = null, Wnd owner = default(Wnd),
		...
		{
			_c.pszWindowTitle = LibUtil.Title(text);
		}
		...
		public TDResult ShowDialog()
		{
			...
			if(_c.pszWindowTitle == null) _c.pszWindowTitle = LibUtil.Title(null);
			...
			try {
				...
				if(hr == 0) {
					...
					LibUtil.DoEventsAndWaitForAnActiveWindow();
				}
			}
			finally {
				_threadIdInShow = 0;
		...
		/// </summary>
		/// <param name="whenClosed">An event handler function (lambda etc) to call (in other thread) when the dialog is closed.</param>
		/// <param name="backgroundThread">Thread.IsBackground property. If true, the dialog disappears when all other threads of this app domain end.</param>
		/// <remarks>Calls <see cref="ThreadWaitOpen"/>, therefore the dialog is already open when this function returns.</remarks>
		public void ShowNoWait(Action<TDResult> whenClosed = null, bool backgroundThread=false)
		{
			//note: not Task.Run, because we need STA, etc.
			var t = new Thread(() =>
			  {
				  ShowDialog();
				  whenClosed?.Invoke(Result);
			...
			t.IsBackground = backgroundThread;
			t.Start();
		...
		{
			Wnd _dlg;
			TaskDialog _tdo; //used only to get _tdo._c for TDApi.TDM.NAVIGATE_PAGE. Using _tdo._hdlg would be unsafe in multithreaded context because may be already set to null, even if caller called IsOpen before.
			...
			/// Example (in an event handler): <c>ed.obj.Send.Message(TaskDialog.TDApi.TDM.CLICK_VERIFICATION, 1);</c>
			...
			/// <summary>
			/// Sends message TaskDialog.TDApi.TDM.SET_ELEMENT_TEXT or TaskDialog.TDApi.TDM.UPDATE_ELEMENT_TEXT to the dialog.
			/// Call this method while the dialog is open, eg in an event handler.
			/// Example (in an event handler): <c>ed.obj.Send.Text(true, TaskDialog.TDApi.TDE.CONTENT, "New text.");</c>
			/// </summary>
			...
			{
				//if(!_IsOpen()) return;
				TaskDialog o = _tdo; if(o == null) return;
	...
	/// <summary>
	/// Dialog buttons, icon, flags and default button. Used as 'style' parameter with <see cref="TaskDialog.Show"/> and similar functions.
	/// <para>
	...
	/// </summary>
	/// <example>
	/// <code>TaskDialog.Show(..., new TDStyle(TDButton.YesNo, TDIcon.Warning, TDFlag.OwnerCenter), ...);</code>
	/// <code>TaskDialog.Show(..., "YN!o", ...); //does the same as the above</code>
	/// <code>TaskDialog.Show(..., TDButton.YesNo|TDButton.Retry, ...);</code>
	/// <code>TaskDialog.Show(..., TDIcon.Warning, ...);</code>
	/// <code>TaskDialog.Show(..., TDFlag.CommandLinks|TDFlag.OwnerCenter, ...);</code>
	/// <code>TaskDialog.Show(..., new System.Drawing.Icon(@"c:\icons\icon.ico", ...);</code>
	/// </example>
		...
		public TDStyle(string style)
		{
			...
			for(int i = 0, n = style.Length; i < n; i++) {
				...
				case 'd':
					...
					default:
						int eon;
						defaultButton = style.ToInt32_(i + 1, out eon);
	...
	/// <summary>
	/// <see cref="TaskDialog.Show"/> return values (selected button, radio button, is checked).
	/// </summary>
	...
	/// <summary>
	/// Edit control type for TaskDialog.ShowInput() and TaskDialog.SetEditControl().
	/// </summary>
	public enum TDEdit
	{
	...
	public partial class TaskDialog
	{
		...
		/// <param name="onLinkClick">Enables hyperlinks in small-font text. A link-clicked event handler function, eg lambda. Example: <c>TaskDialog.ShowEx("", "Text ˂a href="example"˃link˂/a˃.", onLinkClick: ed => { Out(ed.linkHref); });</c></param>
		/// <remarks>
		...
		public static TDResult ShowEx(
			Wnd owner, string text1, object text2 = null,
		...
		{
			var d = new TaskDialog(owner, text1, text2, buttons, icon, flags,
		...
		{
			return ShowEx(owner, text1, text2, buttons, icon, flags, defaultButton, customButtons);
		}
		...
		/// The returned TDResult object has these properties: clicked button id (eg TDResult.OK, 1 (custom button), TDResult.Timeout), name, selected radio button id, check box state.
		/// Tip: TDResult supports implicit cast to int. You can use code <c>switch(TaskDialog.ShowEx(...))</c> instead of <c>switch(TaskDialog.ShowEx(...).Button)</c>.
		/// Tip: For optional parameters use named arguments. Example: <c>TaskDialog.ShowEx("Text.", style: "YN!", title: "Title")</c>
		/// If common and custom buttons are not specified, the dialog will have OK button.
		/// These script options are applied: Script.Option.dialogRtlLayout, Script.Option.dialogTopmostIfNoOwner, Script.Option.dialogScreenIfNoOwner (more info in Wnd.MoveInScreen()), ScriptOptions.DisplayName (title).
		/// If owner, x, y, flag 'owner center', flag 'raw x y' and screen are not specified, the dialog will be in the center of the primary screen or of the screen that contains another window of current process.
		/// Note: the program must have manifest that tells to use comctl32.dll version 6 or later. C# projects by default use old dll, and this function fails.
		...
		{
			var d = new TaskDialog(text1, text2, style, owner,
		...
		/// The returned TDResult object has these properties: clicked button id (eg TDResult.OK, 1 (custom button), TDResult.Timeout), name, selected radio button id, check box state.
		/// Tip: TDResult supports implicit cast to int. You can use code <c>switch(TaskDialog.Show(...))</c> instead of <c>switch(TaskDialog.Show(...).Button)</c>.
		/// Tip: For optional parameters use named arguments. Example: <c>TaskDialog.Show("Text.", style: "YN!")</c>
		/// If common and custom buttons are not specified, the dialog will have OK button.
		/// These script options are applied: Script.Option.dialogRtlLayout, Script.Option.dialogTopmostIfNoOwner, Script.Option.dialogScreenIfNoOwner (more info in Wnd.MoveInScreen()), ScriptOptions.DisplayName (title).
		/// Note: the program must have manifest that tells to use comctl32.dll version 6 or later. C# projects by default use old dll, and this function fails.
		/// </remarks>
		public static TDResult Show(string text1, object text2 = null, TDStyle style = null, Wnd owner = default(Wnd), StringList customButtons = null)
		{
			return ShowEx(text1, text2, style, owner, customButtons);
		}
		...
		/// <param name="style">The same as with <see cref="ShowEx"/>. Can be specified flags and icon, but not buttons (the dialog always has OK and Cancel).</param>
		/// <param name="expandedText">Text that the user can show and hide.</param>
		/// <param name="footerText">Text at the bottom of the dialog. Icon can be specified like "i|Text", where i is: x error, ! warning, i info, v shield, a app.</param>
		/// <param name="title">Title bar text. If omitted, null or "", uses ScriptOptions.DisplayName (default is appdomain name).</param>
		/// <param name="x">X position. 0 - screen center; negative - relative to the right edge.</param>
		...
		/// <c>ed => { if(ed.wParam == 1) { string _s; if(TaskDialog.ShowInputEx(out _s, owner:ed.hwnd)) ed.obj.EditControl.SetControlText(_s); ed.returnValue = 1; } }</c>
		/// Or on OK click it can verify edit control text and don't allow to close the dialog if the text is invalid. Example (lambda):
		/// <c>ed => { if(ed.wParam == TDResult.OK) { string _s=ed.obj.EditControl.Name; if(Empty(_s)) { TaskDialog.Show("Text cannot be empty.", owner: ed.hwnd); ed.returnValue = 1; } } }</c>
		/// </param>
		/// <param name="onLinkClick">Enables hyperlinks in text. A link-clicked event handler function, eg lambda. Example: <see cref="ShowEx"/>.</param>
		/// <remarks>
		/// Uses TaskDialog class instance, like <see cref="ShowEx()"/>.
		/// </remarks>
		public static bool ShowInputEx(
			...
			)
		{
			...
			var d = new TaskDialog(null, staticText, style, owner, customButtons, null,
		...
		{
			return ShowInputEx(out s, out isChecked, checkBoxText, staticText, initText, owner, editType);
		}
			...
			string initText = null, Wnd owner = default(Wnd), TDEdit editType = TDEdit.Number)
		{
			i = 0;
			string s;
			if(!ShowInputEx(out s, out isChecked, checkBoxText, staticText, initText, owner, editType)) return false;
			i = s.ToInt32_();
			return true;
		}
		...
		/// A button-clicked event handler function, eg lambda.
		/// For example, it can set edit control text when a custom button clicked. Example (lambda):
		/// <c>ed => { if(ed.wParam == 1) { string _s; if(TaskDialog.ShowInput(out _s, owner:ed.hwnd)) ed.obj.EditControl.SetControlText(_s); ed.returnValue = 1; } }</c>
		/// Or on OK click it can verify edit control text and don't allow to close the dialog if the text is invalid. Example (lambda):
		/// <c>ed => { if(ed.wParam == TDResult.OK) { string _s=ed.obj.EditControl.Name; if(Empty(_s)) { TaskDialog.Show("Text cannot be empty.", owner: ed.hwnd); ed.returnValue = 1; } } }</c>
		/// </param>
		/// <param name="onLinkClick">Enables hyperlinks in text. A link-clicked event handler function, eg lambda. Example: <see cref="ShowEx"/>.</param>
		...
		{
			bool isChecked;
			return ShowInputEx(out s, out isChecked, null, staticText, initText, owner, editType, style,
		...
		/// Calls <see cref="ShowInputEx"/>.
		/// </remarks>
		public static bool ShowInput(out string s, string staticText = null, string initText = null, Wnd owner = default(Wnd), TDEdit editType = TDEdit.Text)
		{
			return ShowInputEx(out s, staticText, initText, owner, editType);
		}
		...
		/// Calls <see cref="ShowInputEx"/>.
		/// </remarks>
		public static bool ShowInput(out int i, string staticText = null, int initValue = 0, Wnd owner = default(Wnd), TDEdit editType = TDEdit.Number)
		{
			i = 0;
			string s;
			if(!ShowInputEx(out s, staticText, initValue.ToString(), owner, editType)) return false;
			i = s.ToInt32_();
			return true;
		}
	...
	public partial class TaskDialog
	{
		...
		/// <param name="text1">Main instruction. Bigger font.</param>
		/// <param name="text2">Text below main instruction.</param>
		/// <param name="style">The same as with <see cref="ShowEx"/>. Example: "Co" (adds Cancel button and owner-center flag). Example: TDButton.Cancel.</param>
		/// <param name="owner">Owner window or Wnd0.</param>
		/// <param name="checkBox">Check box text. To check, use "Text|true" or "Text|check" or "Text|checked".</param>
		/// <param name="expandedText">Text that the user can show and hide.</param>
		/// <param name="footerText">Text at the bottom of the dialog. Icon can be specified like "i|Text", where i is: x error, ! warning, i info, v shield, a app.</param>
		...
		public static TDResult ShowListEx(
			StringList list, string text1 = null, object text2 = null, TDStyle style = null, Wnd owner = default(Wnd),
			string checkBox = null, string expandedText = null, string footerText = null, string title = null,
			int x = 0, int y = 0, int timeoutS = 0,
			Action<TDEventArgs> onLinkClick = null
			)
		{
			var d = new TaskDialog(text1, text2, style, owner, null, null, checkBox, null, footerText, title, x, y, timeoutS, onLinkClick);
		...
		/// </remarks>
		public static int ShowList(StringList list, string text1 = null, object text2 = null, TDStyle style = null, Wnd owner = default(Wnd))
		{
			return ShowListEx(list, text1, text2, style, owner);
		}
	}
	...
	public partial class TaskDialog
	{
		...
		/// Most parameters are the same as with <see cref="ShowEx"/>. If no buttons specified, adds Cancel.
		/// </summary>
		...
		{
			return ShowProgressEx(marquee, text1, text2, style, owner, customButtons, x: x, y: y);
		}
	}
	...
	public partial class TaskDialog
	{
		...
		/// </summary>
		/// <param name="whenClosed">null or an event handler function (lambda etc) to call (in other thread) when the dialog is closed.</param>
		/// <param name="backgroundThread">Thread.IsBackground property. If true, the dialog disappears when all other threads of this app domain end.</param>
		/// <example><code>
		/// //TaskDialog.ShowNoWaitEx(null, "Text."); //simplest example
		/// var td = TaskDialog.ShowNoWaitEx(ed => { Out(ed); }, "Text.", style: "OCi");
		...
		/// Uses TaskDialog class instance, like <see cref="ShowEx"/>.
		/// </remarks>
		public static TaskDialog ShowNoWaitEx(Action<TDResult> whenClosed,
			string text1, object text2 = null, TDStyle style = null, Wnd owner = default(Wnd),
			StringList customButtons = null, StringList radioButtons = null, string checkBox = null,
			string expandedText = null, string footerText = null, string title = null,
			int x = 0, int y = 0, int timeoutS = 0,
			...
			)
		{
			var d = new TaskDialog(text1, text2, style, owner,
				customButtons, radioButtons, checkBox, expandedText, footerText, title, x, y, timeoutS, onLinkClick);
			d.ShowNoWait(whenClosed, backgroundThread);
			return d;
		}
		...
		/// </remarks>
		public static TaskDialog ShowNoWait(Action<TDResult> whenClosed,
			string text1, object text2 = null, TDStyle style = null, Wnd owner = default(Wnd), StringList customButtons = null,
			bool backgroundThread=false
			)
		{
			return ShowNoWaitEx(whenClosed, text1, text2, style, owner, customButtons, backgroundThread:backgroundThread);
		}
	}
	...
	//{
	//	/// <summary>
	//	/// Shows a scrolling read-only text window where your code then can write text (append lines) like in a console window.
	//	/// Just creates the window and returns. The window runs in other thread.
	//	/// </summary>
