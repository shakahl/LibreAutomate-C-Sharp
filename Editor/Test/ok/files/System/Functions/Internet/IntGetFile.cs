 /
function $url str&s [flags] [inetflags] [dlg] [fa] [fparam] [str&responseheaders] ;;flags: 1-3 cache flags, 4 go online, 8 go online dialog, 16 download to file (s is file name), 32 run in other thread.
...
opt waitmsg -1
Http http
if(!http.GetUrl(url s flags inetflags dlg fa fparam responseheaders)) end F"{ERR_FAILED}  {http.lasterror}"
