 /
function# $items [$text] [$caption] [flags] [hwndOwner] [x] [y] [timeoutS] [default] [`icons] [$dialogIcon] ;;items: "item1[]item2[]...".    flags: 1 don't activate, 2 item IDs, 4 image indices, 8 wider, 16 fixed height, 32 32x32 icons, 64 raw x y, 128 mouse x y.
 ...
 dialogIcon - icon file for dialog title bar icon.
...
if(!empty(dialogIcon)) style|WS_SYSMENU; exStyle~WS_EX_TOOLWINDOW
...
__PlayQmSound 4
#endif
ret ShowDialog(dd &sub.DlgProc 0 hwndOwner (flags&1*128)|(flags&64) iif(flags&128 DS_CENTERMOUSE 0) 0 &z x y dialogIcon)
