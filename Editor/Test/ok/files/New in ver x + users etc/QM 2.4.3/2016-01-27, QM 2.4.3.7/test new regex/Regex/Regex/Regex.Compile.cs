...
int  ec eo rxLen=len(rx)
_p=pcre2_compile(rx rxLen options &ec &eo c)
if(jitOptions and _p) ec=pcre2_jit_compile(_p jitOptions); if(ec) Delete

if !_p
	str regexError.all(4000)
	if(pcre2_get_error_message(ec regexError 4000)<=0) regexError=""; else regexError.fix
	if(eo<rxLen) regexError+F" : {rx+eo}"
	end F"{ec} {regexError}"
