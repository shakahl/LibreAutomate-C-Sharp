 /test SelectFileDialogWithTreeview2
 \Dialog_Editor
function! str&sSelectedFile [$initFolder] [$fileFilter] [hwndOwner]
 ...
 EXAMPLE
 str s
 if SelectFileDialogWithTreeview2(s "C:" "*.txt")
	 out s


if(empty(initFolder)) initFolder="$documents$"
if(empty(fileFilter)) fileFilter="*"
...
foreach(d F"{folder}\*" FE_Dir 2|64)
	str path=d.FullPath
	if(d.IsFolder) aFolders[]=path
	else if(matchw(d.FileName fileFilter 1)) aFiles[]=path
