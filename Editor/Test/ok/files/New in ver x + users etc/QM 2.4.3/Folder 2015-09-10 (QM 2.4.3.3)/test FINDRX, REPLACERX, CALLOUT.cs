out
str s="one two three"
 out findrx(s " \w+" 0 0 _s)

 FINDRX fr.fcallout=&sub.Callout; fr.paramc=5
 out findrx(s " (?C1)\w+(?C2)" fr 0 _s)
...
REPLACERX rr
 rr.repl="M"
rr.fcallout=&sub.Callout; rr.paramc=5
rr.frepl=&sub.Callback_str_replacerx; rr.paramr=77
s.replacerx(" (?C1)\w+(?C2)" rr)
out s


#sub Callout
function[c]# CALLOUT&r
out r.frx.paramc
