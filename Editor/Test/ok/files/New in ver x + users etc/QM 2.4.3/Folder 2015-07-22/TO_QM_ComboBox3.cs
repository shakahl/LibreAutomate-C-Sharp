...
sel message
	...
	case WM_LBUTTONDOWN if((d.style&ES_READONLY and !GetMod) or sub.IsCursorInButton(d lParam)) ret sub.ShowPopup(d)
	...
	case [WM_SETFOCUS,WM_KILLFOCUS] if(d.style&ES_READONLY) goto afterBase ;;don't need caret etc; we'll draw focus rect in nonclient if need
	case else
