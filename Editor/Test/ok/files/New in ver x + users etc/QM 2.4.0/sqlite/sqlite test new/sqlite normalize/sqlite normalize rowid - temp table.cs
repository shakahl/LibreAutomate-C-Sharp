...
x.Exec("CREATE TEMP TABLE temp_texts AS SELECT text FROM texts")
x.Exec("DELETE FROM texts")
PN
x.Exec("INSERT INTO items(pid,ord,flags,date,name,trigger,image,link) SELECT * FROM temp_items")
x.Exec("UPDATE items SET id=0 WHERE id=1")
PN
x.Exec("INSERT INTO texts(text) SELECT * FROM temp_texts")
PN
x.Exec("END")
PN;PO
