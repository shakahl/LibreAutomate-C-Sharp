function [`sheet] [flags] [$book] [`context] [readOnly] [$password] ;;flags: 1 activate sheet, 2 activate book, 4 open or new file, 8 open or new file in new Excel process, 16 make Excel visible, don't close, activate, 0x100 load addins, 0x200 load startup files
 ...
 flags:
   1 - activate the sheet and its worbook.
 ...
 On Windows Vista/7/8, the macro process (QM or exe) should run with same integrity level (IL) as Excel.
...
if sb.len
	if flags&(4|8)
		...
		if(!b) myWB=1; b=a.Workbooks.Open(sb @ readOnly @ password password TRUE); err end ERR_FILEOPEN
	else
