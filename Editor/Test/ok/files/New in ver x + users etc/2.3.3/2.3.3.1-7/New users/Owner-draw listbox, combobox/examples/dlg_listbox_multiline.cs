...
sel message
	...
	for(i 0 a.len) CB_Add h a[i]
	 ...
	 add this code if the listbox later will be resized, for example using splitters. Otherwise does not repaint correctly. However this will not change height of wrapped items; would need to remove/add items when size changes.
	 h=id(3 hDlg)
