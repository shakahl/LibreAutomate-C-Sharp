 ...
 Clears this variable, so you cannot call ScheduleX functions after, but can call Create.
  ...
  However task will not work if QM is "run as", because then gets token from other user's shell window. Also problems with runAs. Could not find a better way.
...
iFile.Save(0 1)
 err
	 ...
	 iFile.Save(0 1)
	  Windows bug: if impersonated, Save fails if was admin task. Although Create2 deleted it, but it seems scheduler service needs some time between deleting and creating task with same name again.
