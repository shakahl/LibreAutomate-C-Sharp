...
namespace Editor
{
	...
	{
		MenuStrip _tsMenu; //menu bar
		CatToolStrip _tsFile, _tsEdit, _tsRun, _tsTools, _tsHelp, _tsCustom1, _tsCustom2; //toolbars
		ToolStripDropDownMenu _ddNew;
		...
		void _Strips_Init()
		{
			...
			_cmd = new _Commands(this);
			//p.Next();

			var builder = new GStripManager(this, Folders.ThisApp + "CmdStrips.xml", Folders.ThisAppData + "CmdStrips.xml", _cmd);
			builder.BuildAll();
			//p.Next();

			//get top-level toolstrips (menu bar and toolbars)
			_tsMenu = builder.MenuBar;
			_tsFile = builder.Toolbars["File"];
			_tsEdit = builder.Toolbars["Edit"];
			_tsRun = builder.Toolbars["Run"];
			_tsTools = builder.Toolbars["Tools"];
			_tsHelp = builder.Toolbars["Help"];
			_tsCustom1 = builder.Toolbars["Custom1"];
			_tsCustom2 = builder.Toolbars["Custom2"];


			//get submenus that will be filled later etc
			builder.Submenus["File_Templates"].Opening += _Strips_MenuOpening_Templates;
			builder.Submenus["File_RecentCollections"].Opening += _Strips_MenuOpening_RecentCollections;
			_ddNew = builder.Submenus["File_New"]; _ddNew.Opening += _Strips_MenuOpening_New;
			builder.Submenus["Tools_Panes"].Opening += (se, da) => _dock.AddShowPanelsToMenu(se as ToolStripDropDown, false, true);
			builder.Submenus["Tools_Toolbars"].Opening += (se, da) => _dock.AddShowPanelsToMenu(se as ToolStripDropDown, true, true);

			//p.NW();

			var k = builder.Submenus["Menu_File"].Items[5] as ToolStripMenuItem;
			k.ShortcutKeys = Keys.Control | Keys.P;
		...
		/// </summary>
		void _Strips_MenuOpening_RecentCollections(object sender, CancelEventArgs e)
		{
			var dd = sender as ToolStripDropDownMenu;
			dd.SuspendLayout();
			//dd.Items.Clear();
		...
		partial class _Commands :IGStripManagerCallbacks
		{
			...
			{
				return EResources.GetImage(imageName);
			}
			...
			internal _Commands(MainForm form)
			{
				_form = form;
				_onClick = _OnClick;
