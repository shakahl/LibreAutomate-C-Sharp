...
namespace WeifenLuo.WinFormsUI.Docking
{
	...
	public partial class DockPane :UserControl, IDockDragSource
	{
		...
		bool _IsVertical(ref Rectangle r)
		{
		...
		private Rectangle CaptionRectangle
		{
			get
			{
				...
				x = rectWindow.X;
				y = rectWindow.Y;
				if(_IsVertical(ref rectWindow)) {
					height = rectWindow.Height;
					width = CaptionControl.MeasureHeight();
				} else {
		...
		internal protected virtual Rectangle ContentRectangle
		{
			get
			{
				...
				int x = rectWindow.X, y = rectWindow.Y, width = rectWindow.Width, height = rectWindow.Height;

				//Catkeys.Output.WriteList(this.CaptionText, IsVertical);
				if(_IsVertical(ref rectWindow)) {
					x += rectCaption.Width;
					width -= rectCaption.Width;
				} else {
					y += rectCaption.Height;
					height -= rectCaption.Height;
				}
		...
		private Rectangle TabStripRectangle_ToolWindow
		{
			get
			{
				...
				if(rectCaption.Contains(x, y)) {
					if(_IsVertical(ref rectWindow)) x = rectCaption.X + rectCaption.Width;
					else y = rectCaption.Y + rectCaption.Height;
				}
