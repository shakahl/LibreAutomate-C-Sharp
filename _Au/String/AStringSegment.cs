...
namespace Au
{
	...
	/// 
	/// One of ways to create <b>AStringSegment</b> instances is to split a string with <see cref="AExtString.Segments(string, string, SegFlags)"/>.
	/// 
	...
	public struct AStringSegment : IEquatable<AStringSegment>, IEquatable<string>
	{
		...
		public AStringSegment(string buffer)
		{
			_buffer = buffer;
			_offset = 0;
		...
		public AStringSegment(string buffer, int offset, int length)
		{
		...
		/// <param name="offset">The offset of the substring in <i>buffer</i>.</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public AStringSegment(string buffer, int offset) : this(buffer, offset, buffer.Length - offset) { }
		...
		/// Uses a <see cref="Util.AStringCache"/>.
		/// </remarks>
		public string ValueCached => Util.AStringCache.LibAdd(_buffer, _offset, _length);
		...
		///
		public override int GetHashCode() => Util.AHash.Fast(_buffer, _offset, _length);
		...
		/// <summary>
		/// Returns true if <i>obj</i> is <b>AStringSegment</b> and its value is equal to that of this variable.
		/// Compares only substrigs, not offsets.
		/// </summary>
		public override bool Equals(object obj) => obj is AStringSegment && _Equals((AStringSegment)obj, false);
		...
		public bool Equals(AStringSegment other) => _Equals(other, false); //IEquatable<AStringSegment>
		...
		/// </summary>
		/// <param name="other">A variable to compare with this variable.</param>
		public bool Eqi(AStringSegment other) => _Equals(other, true);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		bool _Equals(AStringSegment s, bool ignoreCase)
		{
			int len = s.Length;
			if(len != _length) return false;
			return AExtString.LibEq(_buffer, _offset, s._buffer, s._offset, len, ignoreCase);
		}
		...
		/// Compares only substrigs, not offsets.
		/// </summary>
		public static bool operator ==(AStringSegment left, AStringSegment right) => left._Equals(right, false);

		/// <summary>
		/// Compares two substrings and returns true if their values are not equal.
		/// Compares only substrigs, not offsets.
		/// </summary>
		public static bool operator !=(AStringSegment left, AStringSegment right) => left._Equals(right, true);

		/// <summary>
		/// Creates a new <b>AStringSegment</b> from the given string.
		/// </summary>
		/// <param name="value">The string. Cannot be null.</param>
		public static implicit operator AStringSegment(string value) => new AStringSegment(value);

		//public static explicit operator string(AStringSegment seg) => seg.Value;
		...
		/// <param name="cached">Use a <see cref="Util.AStringCache"/>, to avoid much garbage when identical substring values are frequent.</param>
		...
		public string Substring(int offset, int length, bool cached = false)
		{
			...
			offset += _offset;
			if(cached) return Util.AStringCache.LibAdd(_buffer, offset, length);
			return _buffer.Substring(offset, length);
		}

		/// <summary>
		/// Gets a substring of this substring, as <b>AStringSegment</b>.
		/// </summary>
		/// <param name="offset">The start index in this substring.</param>
		public AStringSegment Subsegment(int offset)
		{
		...
		/// <param name="offset">The start index in this substring.</param>
		/// <param name="length">The number of characters.</param>
		public AStringSegment Subsegment(int offset, int length)
		{
			if(offset < 0 || offset + length > _length) throw new ArgumentOutOfRangeException(); //the rest will be validated in the called function

			return new AStringSegment(_buffer, offset + _offset, length);
		}
		...
		/// <summary>
		/// Returns a <see cref="SegParser"/> that will split this substring into substrings as <b>AStringSegment</b> variables when used with foreach.
		/// </summary>
		/// <param name="separators">Characters that delimit the substrings. Or one of <see cref="SegSep"/> constants.</param>
		/// <param name="flags"></param>
		/// <seealso cref="AExtString.Segments(string, string, SegFlags)"/>
	...
	{
		/// <summary>
		/// This function can be used with foreach to split this string into substrings as <see cref="AStringSegment"/> variables.
		/// </summary>
		/// <param name="t"></param>
		/// <param name="separators">Characters that delimit the substrings. Or one of <see cref="SegSep"/> constants.</param>
		/// <param name="flags"></param>
		...
		/// <summary>
		/// This function can be used with foreach to split the specified part of this string into substrings as <see cref="AStringSegment"/> variables.
		/// </summary>
		/// <param name="t"></param>
		...
		{
			var seg = new AStringSegment(t, startIndex, length);
...
namespace Au.Types
{
	...
	/// <summary>
	/// Splits a string or <b>AStringSegment</b> into substrings as <see cref="AStringSegment"/> variables.
	/// </summary>
	/// <remarks>
	...
	public struct SegParser : IEnumerable<AStringSegment>, IEnumerator<AStringSegment>
	{
		...
		/// <summary>
		/// Initializes this instance to split a <see cref="AStringSegment"/>.
		/// </summary>
		/// <param name="seg">The AStringSegment.</param>
		/// <param name="separators">A string containing characters that delimit substrings. Or one of <see cref="SegSep"/> constants.</param>
		/// <param name="flags"></param>
		public SegParser(in AStringSegment seg, string separators, SegFlags flags = 0)
		{
			_separators = separators;
			_s = seg.Buffer;
			_sStart = seg.Offset;
...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
		...
		IEnumerator<AStringSegment> IEnumerable<AStringSegment>.GetEnumerator() => this;

		IEnumerator IEnumerable.GetEnumerator() => this;

		[NoDoc]
		public AStringSegment Current => new AStringSegment(_s, _start, _end - _start);
