...
namespace Au
{
	...
	public static partial class AClipboard
	{
		...
		struct _DisableClipboardHistory
		{
			...
			public void Disable() {
				//if (!AVersion.MinWin10_1809 || AVersion.Is32BitProcessAnd64BitOS) return; //no, let's block clipboard viewers on all OS
				if (AVersion.Is32BitProcessAnd64BitOS) return;
				//if (AKeys.IsScrollLock) return;
				_hh = Cpp.Cpp_Clipboard(default);
				Debug.Assert(_hh != default);
				//AOutput.Write(++s_nhooks); //max 8 when all delays removed in script. Max 4-5 with default options. Max 3-4 with 10.ms() in loop.
			}
			...
			public void Disable() {
				if (!AVersion.MinWin10_1809) return;
				for (AWnd w = default; ;) {
