...
namespace G.Controls
{
	public partial class GPanel :Panel
	{
		...
		public int CaptionHeight
		{
			...
			{
				if(_captionHeight == 0) {
					_captionHeight = (this is MultiGPanel) ? 8 : 19;
				}
		...
		{
			if(IsFloating && !_floaty.Visible) _floaty.Show(_manager.Host);
			base.Show();
		...
		{
			if(IsFloating && _floaty.Visible) _floaty.Hide();
			base.Hide();
			//OnX will set splitter
		}
