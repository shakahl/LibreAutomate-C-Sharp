...
namespace G.Controls
{
	public partial class GPanel :Panel
	{
		...
		/// </summary>
		protected class Floaty :Form
		{
			// the original state of the panel. Used to reset a control to its original state if it was floating
			struct _DockState
			{
			...
			internal DockStyle SavedDockStyle
			{
				get { return _dockState.dockStyle; }
				set { _dockState.dockStyle = value; }
			}

			internal void CopyDockStateFrom(Floaty f) { _dockState = f._dockState; }

			#region overrides
			protected override void OnResizeEnd(EventArgs e)
			{
				if(_isFloating && _isMoving) {
					if(_manager.Overlay.Visible == true && _manager.Overlay.DockHostControl != null) //ok found new docking position
					{
						var parent = _manager.Overlay.DockHostControl;

						var target = parent as GPanel;
						if(target != null && target != _manager.Host && !(target is MultiGPanel)) {
							parent = new MultiGPanel(target);
						}
			...
			{
				//is _MultiDockPanel?
				foreach(var p in _manager.Panels) {
					if(p != _panel && (p is MultiGPanel) && p.Visible && _FindDockHost_FormIsHit(p, pt)) return p;
				}
				//is GPanel?
				foreach(var p in _manager.Panels) {
					if(p != _panel && !(p is MultiGPanel) && p.Visible && !p.IsFloating && _FindDockHost_FormIsHit(p, pt)) return p;
				}
				//is form?
			...
			internal void MakeFloating(bool startMove)
			{
				if(!_isFloating) {
					...
					this.Font = _panel.Font; //GPanel.Font, which is used to draw caption, gets font of parent form, ie this
					...
					_floatingRect = this.Bounds;
				}

				if(startMove) {
					_isMoving = true;
					((Wnd)this).Send(Api.WM_SYSCOMMAND, Api.SC_MOVE | 0x02, 0); //info: modal loop until window dropped
					_isMoving = false;
				}
			}
			...
			internal void MakeDocked(bool dropped)
			{
				...
				if(!dropped) _floatingRect = this.Bounds; //to restore later on "Float" command. If dropped, must save when started to drag.
